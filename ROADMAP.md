...
| `plan.externalized`                                                                                                                                                                               | scaffold | long-horizon ablation            | **keep-gated (v0.119)** — mechanics + harness-seeded creation (plan-mode approval → parsed plan → re-injection, zero model cooperation) shipped; voluntary adoption 0/~10 across 5 families; no eval-scale headroom exists (capable models pass barehanded, llama3.2 is sub-floor: 0%→0% seeded). Evidence venue: a scaffold-on-plan arm in the next SWE campaign |
