...
- **Features**: [`@sidecar` Native Chat Participant](#sidecar-native-chat-participant) · [Zero-Latency Local Autocomplete via Speculative Decoding](#zero-latency-local-autocomplete-via-speculative-decoding) · [Process Lifecycle Hardening](#process-lifecycle-hardening--managedchildprocess--registry--orphan-sweep) (runtime-correctness beat)
- **Refactor beat**: Runtime correctness pass — **(1)** Process Lifecycle Hardening: a unified `ManagedChildProcess` wrapper at `src/agent/processLifecycle.ts` with a deterministic close chain (`graceful close → 2s → SIGTERM → 1s → SIGKILL`), a `ProcessRegistry` singleton pushed into `context.subscriptions`, per-session PID manifest at `.sidecar/pids.json`, startup orphan sweep against the prior session's manifest + owning-cmdline check, and a port-lock sweep for HTTP/SSE MCPs. Every spawn site (MCP stdio, `ShellSession`, custom tools) routes through the wrapper; `MCPManager.disconnect()` gains the per-server timeout that today lets a hung SDK `close()` block extension deactivate. **(2)** FIM + completion subsystem cleanup — draft-model plumbing, `InlineCompletionProvider` consolidation, `completeFIM` signature normalization across backends (paired with the speculative decoding feature above; shared substrate). Both are "make the extension behave correctly under VS Code's lifecycle assumptions" — lifecycle for processes, FIM for completion pipelines.
- **Coverage focus**: `src/completions/provider.ts` + `src/ollama/client.ts completeFIM` path + new `src/agent/processLifecycle.ts` (≥80% per-file by policy; orphan-sweep tests mock the PID manifest + `process.kill(pid, 0)` liveness probe). Maintain ≥80/70/80/80.
- **Acceptance**: `@sidecar` registered as a first-class VS Code chat participant with slash-command parity (`/review`, `/commit-message`, etc.); SideCar backends exposed as `LanguageModelChat` providers for other participants to consume; speculative decoding delivers measured 2–4× tok/s on supported model pairs; a window-reload during an active MCP session leaves zero orphan processes and zero stuck ports on the next activation.
