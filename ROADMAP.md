...
- **Adaptive paste** — intercept paste events in the editor and automatically refactor pasted code to match the current file's conventions: rename variables to match local naming, adjust imports, fix indentation, and replace standard library calls with project utility wrappers. Uses the AST parser and project context to understand what needs adapting
- **Background doc sync** — a "chronicler" that watches for function signature changes, API endpoint modifications, and exported interface updates, then silently updates README.md, JSDoc/docstrings, and OpenAPI/Swagger definitions in a separate git branch. Runs on file save events via the event hook system
- **Zen mode context filtering** — a `/focus <module>` command that restricts the agent's context window to a specific directory or module, ignoring everything else. Dramatically improves speed and accuracy for focused work. Pairs with existing `@pin:path` but inverts the logic (exclude-all-except vs include-specific)
- **Dependency drift alerts** — real-time feedback when `package.json`, `requirements.txt`, `Cargo.toml`, or `go.mod` change. Alert on: bundle size impact, duplicate functionality with existing deps, known vulnerabilities, and excessive transitive dependencies. Trigger via file watcher on dependency manifests
