...
### v0.71 — Live awareness + deferred safety beats
...
- **Deferred folded in** (from v0.69): [Advanced Thinking Visualization & Depth Control](#advanced-thinking-visualization--depth-control) — Live Thinking Panel with four debate modes. · [Scheduled Task Concurrency Safety](#scheduled-task-concurrency-safety--shadow-routing--documentconcurrencygate--deferral-queue) — `documentGate.ts` + `lockPrimitives.ts` + shadow routing so scheduled tasks never land writes on dirty active documents. Folded here because the "safe background execution" theme aligns with this release's push-based subscription work.
...
### v0.63 — Retrieval closing + native backend capabilities ✅ *shipped 2026-04-17*
- **Features shipped**: PKI default-on — `sidecar.projectKnowledge.enabled` flips to `true` (was `false` opt-in since v0.61); critic hash cap prevents runaway re-critique of identical outputs; grep-aware truncation in tool results strips binary noise before the model sees the output. (v0.63.0) · Native backend capabilities (`BackendCapabilities` interface) — `KickstandBackend.nativeCapabilities()` exposes lifecycle (load/unload) + LoRA adapters + model browser; `OllamaBackend` provides a fallback `listLibraryModels` variant. (v0.63.1)
- **Redesign note**: the original v0.63 plan ("First-Class Skills 2.0 — Typed Personas") was redesigned. After building the SkillLoader foundation, the typed-persona concept was reconceived as the Facets system (v0.66) with a richer RPC bus, topological dispatch, and disk-overlay model. `SkillLoader` ships as the distribution layer; persona enforcement ships as Facets.
- **Refactor beat shipped**: `executor.ts` god-function decomposition deferred to v0.69.0 (where it shipped as chunk 1 of v0.69).

### v0.64 — Skills distribution + model routing ✅ *shipped 2026-04-17*
- **Features shipped**: [Skill Sync & Registry](#skill-sync--registry--git-native-distribution-across-machines-and-projects) + [Role-Based Model Routing & Hot-Swap](#role-based-model-routing--hot-swap) — `sidecar.modelRouting.rules` routes each dispatch role (agent / completion / facet / fork) to the configured model; `SkillLoader.initialize()` syncs registry clones from configured remote paths; cost pass-through threads token usage through to the spend tracker. (v0.64.0) · `@xenova/transformers` v2 → `@huggingface/transformers` v4 migration with a Layer 3 embedding-parity harness to gate the transition. (v0.64.1)
- **Refactor beat shipped**: `sidecarFetch` shared helper unifies retry / circuit-breaker / rate-limit / outbound-allowlist across all backend HTTP calls (audit #3 closed).

### v0.65 — Loop ergonomics ✅ *shipped 2026-04-18*
- **Features shipped**: [Steer Queue & Rich Interrupt UI](#steer-queue--rich-interrupt-ui) — FIFO queue with same-urgency coalescing; steerable mid-stream via the existing HookBus interrupt path; `/resume` persistent affordance shipped as chunk 7. · [Multi-File Edit Streams — DAG-Dispatched Parallel Writes](#multi-file-edit-streams--dag-dispatched-parallel-writes) — `runWithCap`-based parallel file-write orchestration with DAG review; pre-dates the `parallelDispatch` extraction in v0.67. · **Bonus** graph-expanded retrieval (chunk 5.5) — `enrichWithGraphWalk` query-time caller-walk added to the `project_knowledge_search` tool ahead of schedule.
- **Refactor beat shipped**: all 14 `src/agent/loop/` helpers unit-tested (audit #6 closed); shared test-helper module (`src/__tests__/helpers/`) in use; bounded `_modelUsageLog` ring-buffer (audit #8 closed).
- **Coverage shipped**: `chatHandlers.ts` 19.63% → ≥80%; subsystem backfill (`scheduler` / `eventHooks` / `lintFix` / `localWorker` / `inlineChatProvider`). Ratchet hit ≥78/68/77/78.
- **Tag**: [`v0.65.0`](https://github.com/nedonatelli/sidecar/releases/tag/v0.65.0) + [`v0.65.1`](https://github.com/nedonatelli/sidecar/releases/tag/v0.65.1) (flaky test fix).
...
### v0.67 — Fork & compare ✅ *shipped 2026-04-18*
- **Features shipped**: [Fork & Parallel Solve](#fork--parallel-solve-multi-path-reasoning) — `dispatchForks()` spawns N agent loops in parallel (each inside its own Shadow Workspace off HEAD), `planForkReview` + `reviewForkBatch` drive the single-winner QuickPick → `vscode.diff` → modal confirm → `git apply` flow. Two user-facing entry points: `/fork <task>` in chat + `SideCar: Fork & Compare` in the palette. [SIDECAR.md Path-Scoped Section Injection](#sidecarmd-path-scoped-section-injection--deterministic-context-scoping-for-small-models) — `parseSidecarMd` splits on H2/H3 boundaries and extracts `<!-- @paths: glob -->` sentinels; the selector routes matching sections by active file + mentioned paths, drops whole sections on overflow instead of mid-chopping. Closes the "15 KB SIDECAR.md burns 3.7 KB of every turn" bloat. Backward-compat: files without sentinels degrade to legacy whole-file behavior. **Kickstand LoRA + model browser** (commit `83b4418` + follow-up coverage closure `904d2f2`) — three new palette commands (Load LoRA Adapter / Unload LoRA Adapter / Browse & Pull Models) wrap the new Kickstand v0.4.0 API.
- **Refactor beat shipped**: `src/agent/parallelDispatch.ts` extracted from duplicated pool-of-N-workers code in `multiFileEdit.ts` + `facetDispatcher.ts`. Typed `runWithCap` + `runForEachWithCap` + `AbortedBeforeStartError` with signal plumbing neither copy had. **Dropped**: Anthropic Batch API folding. After honest audit, the Batches API's ~1-hour async processing + no-streaming + single-request shape doesn't compose with Fork's multi-turn streaming agent-loop model. Stays deferred for a genuine caller (eval harness / planner pass / embedding regen) in a future release.
- **Coverage ratchet shipped**: `src/terminal/errorWatcher.ts` 34.84% → **95.45%** stmts, 100% funcs via an `installShellHarness()` helper that monkey-patches the shell-execution event emitters for end-to-end tests. Plus backfill on Kickstand LoRA surface: `kickstandBackend.ts` 66% → 86%, `backendCommands.ts` 33% → 78%. 3346 total passing across 188 files (+116 from v0.66).
- **Deferred folded in**: The per-fork metrics strip (chunk 4 from the original plan) was dropped — review compares forks by eyeballing diffs, not measurable criteria. Eligible for v0.67.1 or v0.68 if users ask for it. The N-column webview side-by-side review (chunk 5 original scope) was cut to a QuickPick-based pick-the-winner flow — Fork's single-winner semantic doesn't need N columns the way Facets' multi-select does. Hybrid hunk-picking across forks was never in scope for v0.67.
- **Tag**: [`v0.67.0`](https://github.com/nedonatelli/sidecar/releases/tag/v0.67.0). +116 tests, 3346 total (188 files). tsc + lint clean.

### v0.67 deferrals folded into v0.67.1+
- **Kickstand LoRA agent-tool surface** — `kickstand_attach_lora` / `kickstand_detach_lora` / `kickstand_list_loras` tools so the agent can role-shape a model mid-task. Palette commands shipped in v0.67.0; the agent-tool layer is v0.67.1 once we see how users use the palette flow.
- **Per-fork metrics strip** (LOC / tests / guards) — the review UI currently compares by eyeballing diffs; metrics strip adds measurable criteria. Ships if user feedback shows eyeballing is insufficient.
- **N-column side-by-side Fork review webview** — full-scope chunk 5 from the original plan. Pick-the-winner QuickPick is the shipped MVP; webview adds true parallel diff viewing. Eligible for v0.68+ when we know which UX users reach for.
- **Hybrid hunk-picking across forks** — out of scope for v0.67 entirely. Revisit when the metrics strip + webview are shipped.

### v0.68 — GitHub integration maturity ✅ *shipped 2026-04-19*
- **Features shipped**: [CI Failure Analysis & Fix](#ci-failure-analysis--fix--github-actions-log-ingestion-with-proposed-repair-commits) — fetches the latest failed Actions run's log, parses the first error, proposes a repair commit (chunk 4). · [Branch Protection Awareness](#branch-protection-awareness--pre-push-status-check--required-reviewer-warnings) — pre-push warns about required status checks + reviewers (chunk 3). · [Draft PR From Branch](#draft-pr-from-branch--one-command-push--generate--open) — `SideCar: Create Pull Request` handles push + title/body generation + `gh pr create` in one command (chunk 2). · PR review comment fetch + display (chunk 2 of v0.69's plan, shipped early in v0.68's tail). · Agent-powered PR review responses + PR lifecycle mark-ready + CI check snapshot (all shipped in the v0.69.0 batch but planned alongside v0.68).
- **Refactor beat shipped**: `src/review/diffSource.ts` extracted (shared diff-fetching between `prSummary.ts` + `reviewer.ts` + PR-review flow); `src/github/api.ts` error handling unified; 7 previously-untested `GitCLI` methods covered.
- **Deferred from v0.68**: [Merge Conflict Resolution](#merge-conflict-resolution--structural-conflict-marker-handling-in-shadow-worktrees) — the headline feature was not shipped. Structural `<<<<<<<`/`=======`/`>>>>>>>` parsing + Shadow Workspace resolution flow carries to v0.70+. Per-hunk audit review UI (the v0.66 carry-forward) also deferred with it since the two share the hunk-picker surface.
- **Tag**: [`v0.68.0`](https://github.com/nedonatelli/sidecar/releases/tag/v0.68.0).

### v0.68 deferrals folded into v0.70+
- **Merge Conflict Resolution** — `SideCar: Resolve Merge Conflicts` full structural flow: parse conflict markers into hunks, ask the model for a resolution with "ours"/"theirs" context + call-graph awareness, route through a Shadow Workspace so the user can diff before accepting. The per-hunk review UI (from the v0.66 carry-forward) is the prerequisite; both defer together.
- **Line-anchored PR review comment posting** — the fetch + analyze path shipped; the `POST /repos/.../pulls/.../reviews` posting of inline comments is deferred until the authentication story (fine-grained PAT scope) is cleaner.

### v0.69 — Kickstand hardening + GitHub polish ✅ *shipped 2026-04-19*
- **Features shipped**: `executor.ts` decomposition — the 413-line `executeTool` god-function split into `src/agent/executor/` submodules (chunk 1, the v0.63 carry-forward finally landing). · PR lifecycle polish — mark-ready + CI check snapshot (chunk 4). · Groq + Fireworks backend test coverage (chunk 5). · Kickstand reliability wave: n_ctx configurable + Reduce Context button wired (v0.69.7), context-overflow auto-reload (v0.69.7), VRAM auto-evict on 507 error (v0.69.8), SSE EOF detection as error (v0.69.9), MLX model browser filename bug (v0.69.9). · `delegate_task` workers can now run safe read-only shell commands (v0.69.5). · Well-known context lengths for cloud models (v0.69.4). · Token budget exhaustion fix for large tool results (v0.69.2). · Auto-detect loaded Kickstand model on backend switch (v0.69.3). · Cache token double-counting fix in spend tracker.
- **Deferred from v0.69**: [Advanced Thinking Visualization & Depth Control](#advanced-thinking-visualization--depth-control) — Live Thinking Panel with four modes (single / self-debate / tree-of-thought / red-team) deferred to v0.71+; no implementation shipped. · [Scheduled Task Concurrency Safety](#scheduled-task-concurrency-safety--shadow-routing--documentconcurrencygate--deferral-queue) — `documentGate.ts`, `lockPrimitives.ts`, shadow routing for scheduled tasks all deferred to v0.71+; the spec exists but no implementation shipped. · `ollama/types.ts` domain module split deferred.
- **Tag**: [`v0.69.0`](https://github.com/nedonatelli/sidecar/releases/tag/v0.69.0) through `v0.69.9`.

### v0.69 deferrals folded into v0.71+
- **Advanced Thinking Visualization** — Live Thinking Panel: four debate modes, Steer Queue integration, persistent traces at `.sidecar/thinking/<task-id>.md`. Deferred because the `thinking` content block UX needs its own design iteration.
- **Scheduled Task Concurrency Safety** — `src/agent/documentGate.ts` + `src/agent/lockPrimitives.ts`; shadow routing for scheduled tasks so a scheduled refactor never lands a `WorkspaceEdit` on a dirty active document. Deferred alongside v0.70 process lifecycle work — the two share the "safe background execution" theme and should ship together.

### v0.70 — Native VS Code integration + runtime correctness ✅ (in progress)
- **Shipped**: `@sidecar` Native Chat Participant (`/review`, `/fix`, `/explain`, `/commit-message`) · Merge Conflict Resolution (`SideCar: Resolve Merge Conflicts` command, `conflictDetector` + `conflictResolver` + shadow workspace staging) · HuggingFace GGUF file sizes + context length display · Ollama num_ctx auto-set from model native max (via `/api/show`)
- **Remaining**: [Zero-Latency Local Autocomplete via Speculative Decoding](#zero-latency-local-autocomplete-via-speculative-decoding) · [Process Lifecycle Hardening](#process-lifecycle-hardening--managedchildprocess--registry--orphan-sweep) · [Hook Execution Hardening](#hook-execution-hardening--streaming-spawn--activity-adaptive-timeouts--unified-env-sanitization) (runtime-correctness beats)
- **Features**: [`@sidecar` Native Chat Participant](#sidecar-native-chat-participant) · [Zero-Latency Local Autocomplete via Speculative Decoding](#zero-latency-local-autocomplete-via-speculative-decoding) · [Merge Conflict Resolution](#merge-conflict-resolution--structural-conflict-marker-handling-in-shadow-worktrees) (from v0.68 deferrals) · [Process Lifecycle Hardening](#process-lifecycle-hardening--managedchildprocess--registry--orphan-sweep) · [Hook Execution Hardening](#hook-execution-hardening--streaming-spawn--activity-adaptive-timeouts--unified-env-sanitization) (runtime-correctness beats)
- **History note**: Process Lifecycle Hardening and Hook Execution Hardening were fully implemented in commits d9d9d1f–8f5319a (2026-04-17) but reverted via commit 595df58 ("defer v0.70 process lifecycle features to next release") before v0.69.5 shipped. The implementations exist in git history as reference; the revert was motivated by integration concerns with the existing spawn sites. They land here in v0.70 after those concerns are resolved.
- **Refactor beat**: Runtime correctness pass — **(1)** Process Lifecycle Hardening: a unified `ManagedChildProcess` wrapper at `src/agent/processLifecycle.ts` with a deterministic close chain (`graceful close → 2s → SIGTERM → 1s → SIGKILL`), a `ProcessRegistry` singleton pushed into `context.subscriptions`, per-session PID manifest at `.sidecar/pids.json`, startup orphan sweep against the prior session's manifest + owning-cmdline check, and a port-lock sweep for HTTP/SSE MCPs. Every spawn site (MCP stdio, `ShellSession`, custom tools) routes through the wrapper; `MCPManager.disconnect()` gains the per-server timeout that today lets a hung SDK `close()` block extension deactivate. **(2)** Hook Execution Hardening: unified `src/agent/hookRunner.ts` replaces `execAsync` in both `sidecar.hooks` (per-tool pre/post) and `sidecar.eventHooks` (onSave/onCreate/onDelete) with a streaming `spawn`-based implementation — bounded output buffer (default 10 MB, truncates cleanly with `[... N bytes elided]` instead of crashing), activity-adaptive timeout (initial 15s, extends +5s per chunk of stdout/stderr, cap 5 min), unified `sanitizeEnvValue()` extracted to `src/agent/envSanitize.ts`. Hook children route through the `ManagedChildProcess` wrapper so orphan sweep covers them too. **(3)** FIM + completion subsystem cleanup — draft-model plumbing, `InlineCompletionProvider` consolidation, `completeFIM` signature normalization across backends (paired with speculative decoding).
- **Coverage focus**: `src/completions/provider.ts` + `src/ollama/client.ts completeFIM` path + new `src/agent/processLifecycle.ts` + new `src/agent/hookRunner.ts` + new `src/agent/envSanitize.ts` (≥80% per-file by policy). Maintain ≥80/70/80/80.
- **Acceptance**: `@sidecar` registered as a first-class VS Code chat participant with slash-command parity (`/review`, `/commit-message`, etc.); SideCar backends exposed as `LanguageModelChat` providers for other participants to consume; speculative decoding delivers measured 2–4× tok/s on supported model pairs; a window-reload during an active MCP session leaves zero orphan processes and zero stuck ports on the next activation; a hook producing 50 MB of stdout truncates at the configured cap without crashing the agent loop; `SideCar: Resolve Merge Conflicts` parses `<<<<<<<`/`=======`/`>>>>>>>` markers, routes resolution through a Shadow Workspace.
...
Twenty-four-track audit launched after v0.79.0. All 24 tracks completed 2026-04-21. Findings folded into v0.80 refactor beat and individual backlog items below.
