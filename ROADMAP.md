...
Last updated: 2026-04-15 (v0.54.0 released — `PolicyHook` interface + `HookBus` closes the last cycle-2 HIGH architectural deferral, and two new OpenAI-compatible providers ship as empty-body subclass wrappers to prove the v0.53 anticorruption layer pays off: Groq (LPU inference at thousands of tokens/sec) and Fireworks (cheap open-weight hosting). **Cycle-2 architectural audit is now fully closed.**)
...
- **GPU-Native Hot-Swapping** — tiered model dispatch based on task complexity. S...
