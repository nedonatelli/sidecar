...
| `sidecar.redCheckGate.pythonRunners`         | boolean | `false` | EXPERIMENTAL. Let a failing Python test runner or reproduction script run through `run_command` (`runtests.py`, `manage.py test`, `python -m unittest`, `pytest`, `python repro.py`) arm the red-check gate, the way `run_tests` always has. Off: measured with the keep-best ratchet off, forcing retries on a red check made the patch worse |
| `sidecar.redCheckGate.ratcheted`             | boolean | `false` | EXPERIMENTAL. Treat the agent's response to a red-check refusal as scaffold-tail work: the keep-best ratchet snapshots before the retry and reverts it unless the verification signal improved. Requires the keep-best ratchet (`scaffolding.keepBest`)                                                                                        |
...
| `sidecar.retrieval.cliffGate`         | boolean     | `true`  | Trim workspace retrieval at its similarity cliff: drop hits below a sharp score drop, and reduce a result with no drop at all to its single best hit. Tightly-packed scores mean the extra hits are near-identical siblings of the right answer rather than new information, which measurably hurts smaller models. |
