...
- **Cycle detection** — detects repeating tool-call patterns and halts to prevent infinite loops. Length-1 patterns (the same call repeated) require 10 normalized repeats to trip (configurable via `sidecar.scaffolding.cycleDetectionMinRepeats`; the exact-signature check fires one repeat later, and repeated writes to one file trip at 6), so agents can legitimately re-run a tool to verify after edits or retry tests after fixes without getting cut off. Length 2..4 patterns (A,B,A,B or A,B,C,A,B,C) still trip after two full cycles, since they're a much clearer signal of a stuck loop.
...
- **Local-model call recovery** _(v0.119)_ — deterministic fixes for the malformed calls small models emit, each disclosed in the tool result so the model learns the correct form: wrong parameter names are remapped (`file`→`path`, `old_string`→`search`), foreign tool names from other assistants' catalogs are aliased (`create_file`→`write_file`, `bash`→`run_command` — aliases inherit the canonical tool's approval gating), `edit_file` on a file that doesn't exist with creation intent executes as `write_file`, and tool calls pasted as bare JSON text are parsed and dispatched.
- **Degenerate-output protection** _(v0.119)_ — corrupted model output (repeated reserved tokens, sampler loops) is discarded instead of standing as a final answer; the agent retries once and stops cleanly if it recurs.
- **Externalized planning** _(v0.119, opt-in `sidecar.plan.externalized`)_ — plans approved in plan mode are re-injected each turn so they survive context compression; the harness advances the plan automatically as planned files are written, and the completion gate refuses to finish while plan-named deliverables are missing.
...
Select custom modes from the dropdown alongside built-in modes (cautious, autonomous, manual, plan, review, audit). Custom modes appear with an orange badge.
