...
The right instrument is a **count/rate**: _unresolved-citation count per run_, compared as means across arms — V1's lift then reads as "fewer fabrications with the gate" (e.g. 2.1 → 0.4 avg). The scorer already computes the unresolved set; the work is exposing the count as a numeric ablation metric instead of collapsing to a boolean. Deferred (it's a project of its own); `citationsResolve` is meanwhile kept as a **soft** expectation so the case isn't a permanent red.
