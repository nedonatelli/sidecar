...
- **S1. Plan externalization ("plan-in-the-harness")** _(L, mechanics shipped v0.119 — adoption evidence open)_ — `update_plan` tool + per-turn `<plan_state>` re-injection (`{current step, last result, remaining steps}`), compaction-proof by construction; plan persists into the crash-resume checkpoint; pre-plan nudge in the per-turn addon slot (cache-safe). Probing found + fixed a real pre-existing bug: the tool catalog and dispatch ignored per-run config overrides (injection-first violation), so config-gated tools were invisible to facet/eval runs. **Measured negative (v0.119, test-across-models):** voluntary adoption is ZERO — 0/8 nudged plan-arm runs across three model families (qwen2.5-coder:7b, ministral-3, qwen3.5) called `update_plan`, even on a discriminative case (non-derivable step codes, instructions-via-read, a forced-compression 40KB-noise step). And no lift is measurable at eval scale: all 8 runs passed in BOTH arms — local models handle 10 sequential steps in ~12 tool calls; the long-horizon failure regime S1 targets lives at SWE-length runs, not eval-case scale. **Design implication for the next slice:** plan creation must be harness-seeded, not model-volunteered — the natural source is plan-mode output (an explicit, user-approved plan text, parseable deterministically) seeding the external plan on approval. The re-injection machinery is sound and tested; the adoption path is what needs replacing. Feature stays default-off; Ledger: keep-gated with this evidence.
