...
The default model is `gemma4:e4b` (~10 GB VRAM). For lighter hardware, try `ministral-3:latest` (6 GB), `granite4.1:3b`, or `qwen2.5-coder:7b`. For measured agent ratings across local models — including which ones to avoid for agent mode — see [Model Compatibility](model-compatibility).
...
2. **Recommended:** click the ⚙ gear → **Kickstand** in the chat header.
