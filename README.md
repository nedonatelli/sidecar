...
- **True agentic autonomy** — SideCar reads your code, edits files, runs tests, reads the errors, and iterates until the task is done. Switch between cautious, autonomous, manual, plan, review, audit, or custom user-defined modes.
...
- **Fork & Parallel Solve** *(new in v0.67)* — `/fork <task>` or `SideCar: Fork & Compare` spawns N parallel approaches to the same task, each running a full agent loop inside its own Shadow Workspace off the current `HEAD`. Every fork forces `forceShadow: true, deferPrompt: true` so the main tree stays untouched during the run and no mid-run quickpicks fire. When every fork settles, a pick-the-winner QuickPick lists the reviewable forks (files touched + duration), `vscode.diff` opens the winner's patch, a modal confirms apply, and `git apply` lands the result while the losing forks are discarded silently. Fork runs the **same** task N ways (differs from Facets, which dispatches N specialists on different subtasks). Tool-call events from each fork are tagged `fork-<n>:` so a future webview can route them to the right column. Aborts propagate cooperatively via `AbortedBeforeStartError` — unstarted forks surface as typed results, never silent omissions. Config: `sidecar.fork.defaultCount` (default `3`, clamp 2–10), `sidecar.fork.maxConcurrent` (default `3`, clamp 1–10)
- **SIDECAR.md path-scoped section injection** *(new in v0.67)* — replaces the legacy whole-file-dump behavior with a deterministic, path-aware selector. Sections opt-in to scoping via an HTML-comment sentinel immediately under the H2 heading (`<!-- @paths: src/transforms/**, src/dsp/** -->`, invisible in GitHub's markdown preview). SideCar injects always-include sections (no sentinel + user-declared heading list) + scoped sections whose globs match the active editor or user-mentioned paths, dropping whole sections in reverse priority order on overflow instead of mid-chopping. Closes the "15 KB SIDECAR.md burns 3.7 KB of every turn on a 4K local Llama regardless of relevance" bloat. Backward-compat: files without any `@paths` sentinels degrade to the pre-v0.67 whole-file behavior automatically — no migration needed. Mode override via `sidecar.sidecarMd.mode` (`sections` default | `full` legacy); config-declared always/low heading lists via `sidecar.sidecarMd.alwaysIncludeHeadings` / `sidecar.sidecarMd.lowPriorityHeadings`
- **Safety guardrails** — agent mode dropdown (cautious/autonomous/manual/plan/review/audit/custom) in the header, iteration limits, token budget, daily/weekly spending caps
...
- **DB Write Tools** *(new in v0.80)* — `db_execute` runs parameterised INSERT/UPDATE/DELETE/DDL and buffers writes to `.sidecar/audit/db/` in Audit Mode; `db_migrate_up` drives any migration CLI (Prisma / Knex / Flyway / Liquibase / Goose / Alembic) via `execFile` (no shell injection surface). Both tools require approval. **Security hardening** (v0.80) — SSRF protection for `screenshot_page`, path-traversal guard for `analyze_screenshot`, env-var whitelist for `run_playwright_code`, SQL allowlist in `assertReadOnly`, shell-injection fix in lint runner, and three disposable-leak fixes
- **NotebookLM research mode** *(new in v0.82)* — `/notebook` enters source-grounded chat mode where every answer carries mandatory inline citations. `ingest_source` indexes `.md`/`.tex`/`.rst`/`.pdf` files into a per-session source registry. Five study-aid generators produce fully-cited output: `generate_briefing`, `generate_study_guide`, `generate_faq`, `generate_timeline`, `generate_outline` — written to `.sidecar/research/<project>/generated/`. Uncited claims are flagged `⚠ unsupported` in strict mode. Toggle via `sidecar.notebookMode.enabled`
- **Conversational shortcuts** *(new in v0.81)* — type natural phrases instead of clicking buttons: "yes/sure/go ahead" executes a pending plan; "no/cancel/scratch that" rejects it; any other reply while a plan is pending becomes revision feedback. "undo/revert" rolls back agent file changes; "commit it/lgtm" generates a commit message; "what changed/diff" replays the change summary; "I don't know/your call" tells the agent to use its best judgment and proceed. Numbers select from the agent's last numbered list ("2" picks item 2). Messages that already contain a bulleted or numbered list skip plan-mode auto-trigger.
...
- **Slash commands** — `/model`, `/help`, `/batch`, `/doc`, `/spec`, `/insight`, `/save`, `/sessions`, `/scan`, `/usage`, `/context`, `/test`, `/lint`, `/deps`, `/scaffold`, `/commit`, `/verbose`, `/prompt`, `/audit`, `/insights`, `/mcp`, `/init`, `/move`, `/clone`, `/skills`, `/releases`, `/release`, `/bg`, `/notebook` (source-grounded research mode), `/fork` (parallel-solve N approaches), `/code` (exit Notebook Mode), `/review` (review git changes), `/pr` (push branch + open draft PR), `/pr-summary`, `/pr-respond`, `/pr-ready`, `/pr-ci`, `/review-comments`, `/ci` (analyze failing CI), `/resume` (continue a cut-off response), `/memories` (browse agent memories), `/revise` (revise current plan) — with autocomplete dropdown as you type. Actions that duplicated header buttons or palette commands (`/reset`, `/export`, `/compact`, `/undo`) were removed in v0.47.0; use the buttons or `SideCar:` palette entries instead
...
- **Per-server tool allowlist** — `toolAllowlist: ["tool_a", "tool_b"]` exposes only the listed tools; all others are silently dropped at registration time
- **Per-tool enable/disable** — filter out individual tools per server
...
- **NoSQL databases** *(new in v0.82)* — `SideCar: Install NoSQL MCP Server` pre-configures MongoDB (`mcp_mongodb_find/aggregate/insert_one/…`, requires npx) or Redis (`mcp_redis_get/set/delete/…`, requires uv) with a single QuickPick + connection-string prompt. See [docs/extending-sidecar.md](docs/extending-sidecar.md) for full configs and `toolAllowlist` examples
...
### Tool Registry (59 built-in tools + MCP)
| Tool | Description |
|------|-------------|
...
| `db_execute` *(new in v0.80)* | Run parameterised INSERT/UPDATE/DELETE/DDL; writes buffered in Audit Mode — requires approval |
| `db_migrate_up` *(new in v0.80)* | Run a migration CLI (Prisma / Knex / Flyway / Liquibase / Goose / Alembic) — requires approval |
