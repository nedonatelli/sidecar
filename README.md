...
| **Externalized planning** _(new in v0.119)_ | Opt-in (`sidecar.plan.externalized`): plans parsed from approved plan-mode output survive context compression via per-turn re-injection, the harness advances the plan automatically as deliverables are written, and the completion gate refuses "done" while plan-named files are missing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **Local-model scaffolding harness**         | Capability-adaptive machinery that makes weaker local models usable: a deterministic reliability layer _(new in v0.119)_ — parameter-name recovery (`file`→`path`), foreign tool-name aliases (`create_file`→`write_file`, `bash`→`run_command`), `edit_file` creation-intent coercion, bare-JSON tool-call parsing, and a degenerate-output detector — plus grounded review + citation-resolution gates, constrained-decoding repair of malformed tool calls, capability-tiered reprompt/critic/compression budgets (weak models get more help, strong models run lean), read-only specialist routing, and write/rewrite-thrash defenses. A failure taxonomy buckets every run (`malformed-call` / `wrong-tool` / `timeout` / `incomplete` / `bad-reasoning`) and `npm run eval:ablation` measures each scaffold's pass-rate lift vs. latency so pure-tax scaffolds get cut |
