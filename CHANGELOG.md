...
### Stats
- 8860 total tests (496 test files)
...
### Fixed (pre-release truth audit)

- **A six-agent audit verified every claim surface against the code — docs site, README/SECURITY/CONTRIBUTING, setting descriptions, tool descriptions, the base system prompt, harness reprompts, UI strings, and in-code comments — and fixed ~100 verified-false claims.** The worst were model-facing: the base prompt advertised `fetch_url`/`create_pr`/`review_pr` (none exist) including a routing rule instructing the model to chain a nonexistent tool; a completion-gate reprompt said to "call done again" (no such tool); the notebook `generate_*` tools claimed to synthesize documents while writing placeholder skeletons, with examples that omitted their required parameter; `db_list_connections` printed display names while every sibling tool requires ids. User-facing: the Seatbelt sandbox was described as confining every shell command (it covers only the fallback executor), a pre-commit secret scan was documented that has always been manual, `sidecar.thinking.mode`'s enum descriptions promised model behaviors that don't exist, and the README inverted the `mcpServer.requireAuth` security default.
- **Behavior fixed where the claim deserved to be true:** the two documented-but-never-invoked `PolicyHook` phases (`beforeIteration`, `onTermination`) are now wired, so SDK hooks using them fire; `/sandbox <task>` is wired end-to-end and `shadowWorkspace.mode: always` now actually applies to interactive chat; the agent's own terminal is now excluded from the terminal-error watcher (the ignore list named the wrong terminal); `run_playwright_code`'s documented `playwright-core` import now resolves; `/help` is generated from the command list so it can never drift again; `/notebook` refuses activation with guidance when its tool group is disabled instead of promising tools the model cannot see; `recovery.codeAsText`, `editFile.steerToWrite(+Threshold)` are now declared in Settings; `describeScaffold` no longer stamps `keepBestRatchet: false` into provenance manifests for runs that used it, and the SWE bench arms pin the ratchet flag explicitly (the "bare" arm had silently inherited it since the v0.118 default flip).

### Fixed

- **Tree-sitter never loaded in a packaged install — every install parsed with the regex fallback.** The analyzer was reached through an indirected dynamic import written to stop static resolution, which also stopped esbuild from bundling it: `dist/` holds only `extension.js`, so the runtime specifier threw `ERR_MODULE_NOT_FOUND`, and a `.catch(() => null)` inside the try block absorbed exactly the error the outer handler existed to report. The extension shipped 28 MB of grammar `.wasm` it could never load. Confirmed three ways: the VSIX contains no `treeSitterAnalyzer.js`; reproducing the call from `dist/` throws; the live cached graph matches the regex analyzer's symbol count (6,541) and not tree-sitter's (5,836). The specifier is now literal so the module bundles, grammar loading stays lazy, and the new test reads `dist/extension.js` itself — every existing test called the analyzer directly and so passed throughout. (`src/parsing/registry.ts`)
- **Concurrent callers raced past the tree-sitter load and got regex anyway.** The load was gated on a boolean set the instant the first caller arrived; the symbol indexer calls `getAnalyzer` concurrently for every file, so all but the first skipped the load block, found the analyzer still null, and took the regex fallback — while the grammars loaded correctly into a variable nobody was waiting on. Diagnosed from a real install: the graph written the same second the grammars loaded carried 6 `method` symbols against tree-sitter's 1,487 (the regex analyzer emits no `method` symbols at all, which is the categorical tell). The load is now a memoized promise every caller awaits (43ms for 28 extensions). The regression test fires 50 concurrent calls, because one or two do not reproduce it. (`src/parsing/registry.ts`)
- **`GRAPH_VERSION` 5 → 6 so the tree-sitter fix reaches existing caches.** Reconciliation skips files whose content hash is unchanged, and swapping the extractor changes no file's content — so an upgraded install restored its regex-built graph, re-parsed nothing, and kept serving symbols from the parser that was just replaced. (`src/config/symbolGraph.ts`)
- **The regression baseline compared 61 of 70 cases — and the nine it skipped were the hardest ones.** The eval suite and the baseline recorder each assembled their own case list, and they drifted: the `latch-*` multi-turn and `dogfood-*` real-workspace families were never compared, so a model could have lost all multi-turn capability and the baseline would have reported no regression. Both now import one `ALL_AGENT_CASES`, and a test asserts neither assembles its own array — the same single-source fix as the v0.122.2 exclude lists. (`tests/llm-eval/allCases.ts`)
- **An infra throw ended the whole model run instead of one case.** The baseline recorder runs all 70 cases inside a single `it`, so one "operation was aborted" at case 16 ended llama3.2's run — and because the recorder flushes incrementally, the 16 cases it had reached overwrote a complete 69-case baseline. Infra throws are now caught and treated identically to an unavailable backend: skipped, logged, counted toward the circuit breaker; anything else still rethrows. (`tests/llm-eval/agentBaseline.eval.ts`)
- **A timed-out baseline run discarded every measurement it had made.** Three models ran overnight for ~12 hours and produced zero baselines: each hit the 4h outer cap mid-run, and the baseline was written once, after the loop. The cap is now 12h (what the per-case formula asks for before clamping), the file is flushed after every case, and per-case progress is logged — the record path previously printed nothing until it finished. A partial baseline is safe under the provenance guard; cases never reached simply are not compared. (`tests/llm-eval/agentBaseline.eval.ts`)
- **The baseline sweep had no circuit breaker and hung for nine hours against a dead backend.** A power loss wedged Ollama's llama-server: `/api/tags` still answered while generation returned nothing, and the sweep sat on a vitest process that had written nothing for nine hours. Three consecutive empty cases now abort with everything measured already on disk. The sweep driver (`scripts/baseline-sweep.sh`) additionally probes *generation* — not reachability — before each model and stops the whole run when the backend is dead: a reachable `/api/tags` is not evidence Ollama can serve. (`tests/llm-eval/agentBaseline.eval.ts`, `scripts/baseline-sweep.sh`)
- **One network blip reported 16 case regressions.** A case the backend never answered was scored as a fail and then compared against a passing baseline. Unavailable cases are now excluded from the comparison and counted separately. (`tests/llm-eval/agentBaseline.eval.ts`)
- **A filtered re-run polluted the run-history timeline it was meant to protect.** A targeted 4-case re-run recorded `1/4, complete: true`, and a naive trend read 27/69 → 26/70 → 1/4 as a collapse. Entries now carry `filtered` when `SIDECAR_EVAL_CASE`/`SIDECAR_EVAL_TAGS` narrowed the selection; they stay in the file but are not points on a trend. (`tests/llm-eval/baselineHistory.ts`)
- **The per-case eval wall-clock budget was binding on models it was not meant to test: 300s → 600s.** ministral-3 hit the cap on ~29% of its cases, and claude-sonnet-5 hit it on `run-tests-after-fix` — a limit a frontier model reaches is measuring how long a subprocess takes, not separating capable models from incapable ones. It went unnoticed because a cap-hit is not automatically a failure: the workspace is scored as it stands. (`tests/llm-eval/agentHarness.ts`)
- **`tests/` was type-checked by nothing — not `npm run check`, not CI.** `tsconfig.tests.json` was wired into lint-staged alone, which runs only when a `tests/` file is itself staged; CI's typecheck resolves `tsconfig.json`, which does not include `tests/`. A `src/` change that broke a test file's types reached main unseen. Four real defects surfaced when the check first ran. `compile:tests` is now in both `npm run check` and CI, verified by injecting a deliberate type error. (`.github/workflows/ci.yml`, `tsconfig.tests.json`)
- **The integration suite could run against a stale bundle.** `test:integration` launched vscode-test against whatever `dist/` happened to hold; a dogfood run reported five passing tests for code that was not in the build. The runner now asserts build freshness before launching. (`scripts/lib/buildFreshness.mjs`)
- **Eval case quality, from ceiling-validating the corpus:** 17 cases failed a model for choosing an equally valid write tool; the TypeScript fixtures had no `tsconfig.json`, so `tsc` returned 5,301 characters of help text instead of diagnostics (and the fixture check resolved `tsc` from the machine, not the repo); the sandbox glob threw on the exact pattern `run_tests` emits; `latch-stale-fact` failed correct answers for naming the old value; `ask-user-ambiguous-rename` tested half the loop and was unpassable; route-tool pins that the outcome assertion already covered were dropped. (`tests/llm-eval/`)

### Added

- **Baseline run history.** Every baseline run appends one JSON line to `baselines/history.jsonl` — model, provenance, cases run vs. available, pass/fail, unavailable count, duration, failing case ids — written on both the success and the abort path, so a truncated run is visible as `complete: false` instead of silently replacing a complete baseline (which happened twice in one sweep). `timelineFor` and `deltaAgainstPrevious` exclude incomplete and filtered runs; `appendHistory` never throws, because losing telemetry must not fail a run that produced real measurements. (`tests/llm-eval/baselineHistory.ts`)
- **Multi-trial baseline recording.** `SIDECAR_EVAL_TRIALS` now works in record mode: above 1, the recorded `passed` is a majority of trials and the pass rate is stored beside it, so a marginal case is legible in the file rather than arriving later as a phantom regression. Measured need: of eleven cases that flipped between the 08-02 and 08-05 sweeps, five flip on seed alone, and three more contradict a same-seed re-run — multi-turn cases are not reproducible even seeded. (`tests/llm-eval/agentBaseline.eval.ts`)
...
- **Exported constants were invisible to the code graph.** Every top-level `export const` in a workspace — 221 in this repo — was absent from the symbol graph, so `find_references`, `analyze_impact` and semantic retrieval silently under-reported on the registries, model lists, prompt strings and config tables that carry much of the project's behaviour, and an empty result was indistinguishable from a symbol with no references. `SymbolIndexer` already accepted `variable` symbols and both type unions already included them; neither analyzer ever emitted one. Both now do — the tree-sitter mappings gained a `perDeclarator` form that emits one element per bound name (a declaration can bind several, and the names live on nested declarators), and the regex fallback gained equivalent recognition backed by a scanner that steps over strings, comments and regex literals. `GRAPH_VERSION` 3 → 4 so existing caches repopulate; reconciliation is by content hash, so without the bump no file would ever be re-parsed. Found by differential comparison against an independent extractor, not by the suite — every existing test was written against the same extractor that produced the data. (`src/parsing/treeSitterAnalyzer.ts`, `src/astContext.ts`)

- **Any command that reads stdin swallowed the shell's completion sentinel, corrupting its output and fabricating its exit code.** The persistent session signals completion by writing `echo "<SENTINEL>_$?_END"` as a **second line to the shell's stdin**. A command that reads stdin consumes that line instead of the shell executing it, then prints it — measured against the real session, `cat` returned `echo "` as its entire stdout, and the exit code was then parsed out of that echoed text, reporting a confident `0` for a command that never completed. Heredocs were worse: `cat <<EOF` ate the sentinel and the command **hung until the timeout**. `cat`, `python`, `npm init`, `git commit` with no `-m`, `ssh`, and every confirmation prompt were affected, and nothing guarded against it. The command now runs with its stdin redirected from the null device, which is the correct semantics anyway — `run_command` has no way to supply input, so immediate EOF beats blocking forever on a human. The redirect is applied to a **brace group** rather than appended to the command: a bare `<cmd> < /dev/null` binds to the last element of a pipeline and would break `echo x | cat`, and braces rather than a subshell keep `cd` and `export` persisting across calls. The newline before the closing brace is load-bearing — `{ sleep 1 & ; }` is a syntax error, and a trailing `#` comment would swallow a semicolon. Ten regression tests cover pipelines, `cd`/`export` persistence, trailing `&`, trailing comments, heredocs, stderr capture, and exit-code fidelity; three of them fail against the old implementation. (`src/terminal/shellSession.ts`)

- **A third memory store could destroy its file on an unreadable load.** `PinnedMemoryStore` had the same shape as `DurableMemoryStore` and `AgentMemory`: `load()` caught every failure into an empty list, and `persist()` writes the whole collection, so one transient read error plus one pin wiped the user's pinned context. Migrated onto `jsonStore` — quarantine on unreadable, atomic writes, observable failure. Its test file also moved off a `fs/promises` module mock that asserted `writeFile` had been *called*, which pinned the call shape while proving nothing about the bytes and broke the moment persistence became atomic. (`src/agent/memory/pinnedMemory.ts`)

- **Deleting a file corrupted the file-level embedding index, then crashed it.** `removeFile` deleted the entry and recomputed `count` from the entry list, but never compacted the vector store and never re-assigned the offsets of entries after the hole — breaking the invariant every other method relies on (offsets are exactly `0..count-1`, and `vectors` holds exactly `count` rows). The next file to be indexed then took `offset = count`, which **collided with a surviving entry**: the new vector overwrote a live one and semantic search returned the wrong file's embedding, with no error anywhere. After a second deletion the append allocated fewer rows than the array held, and `Float32Array.set` threw `RangeError: offset is out of bounds`, killing the flush as an unhandled rejection — caught live in a real extension host. It also outlived restarts: `persist` writes the bogus offsets along with only `count` rows of vectors, so the next session loaded an index pointing past the end of its own array. `removeFile` now compacts (drop the row, slide the rest down, re-base offsets), appends take their row index from the array rather than a counter that can disagree, and `restoreCache` validates that offsets are dense before trusting a cache — rebuilding costs one re-index, where loading corruption costs wrong search results indefinitely. v0.122.1 fixed this class in the *symbol* index; the *file* index never got the same treatment. (`src/config/embeddingIndex.ts`)

- **`get_diagnostics` reported nothing for the case it exists to serve.** It read `languages.getDiagnostics(uri)`, which only returns what a language server has already published — and servers publish for files in their open set, which never includes a file the agent just wrote to disk. Measured in a real extension host: an unopened file with two deliberate type errors returned **0 diagnostics after 10s**, and `workspace.openTextDocument` alone still returned **0 after 20s**. The tool now opens the file in a preview tab that takes no focus, waits for the analysis, and closes exactly the tab it opened — no dirty buffer, no save, no format-on-save, no focus change. A never-opened broken file now reports its real type error. Budget is configurable via `sidecar.diagnostics.analysisBudgetMs` (default 5000; `0` restores the old cache-only read). An empty result is still never presented as proof a file is clean: there is no signal for "the analysis finished and found nothing" — an empty publish arrived at 1.4s for a file whose real error appeared later — so a positive result is trustworthy and an empty one is not. (`src/agent/tools/diagnostics.ts`)

- **The completion gate accepted an empty `get_diagnostics` result as proof the model had verified its edit.** `lintObserved` was set on the *call*, not the output — the gate-design error the v0.114 dogfood already taught once (key on output evidence, not the invocation). It matters most here because empty is the *normal* result: `languages.getDiagnostics(uri)` reports only on files a language server has already analyzed, and a server analyzes files open in a **visible editor** — which a file the agent just wrote never is. Measured in a real extension host (VS Code 1.126 and 1.131, TS language features present): a file with two deliberate type errors sitting on disk unopened returns **0 diagnostics after 10s**; `workspace.openTextDocument` alone returns **0 after 20s**; only `showTextDocument` produces them (~2s). Matching the audit log, where 30 of 33 real calls returned "No diagnostics" and all three non-empty results came from SideCar's own security scanner rather than a language server. Combined with a tool description that called the result authoritative and told the model *not* to follow up with `tsc`/`eslint`, a model could introduce a type error, call the tool, be told nothing, be forbidden from checking properly, and be credited with verifying its work. The gate now requires output evidence that an analyzer actually ran; the reprompt names what happened and directs to a checker that executes, instead of sending the model back to the tool that just told it nothing; and the tool description and base-prompt rule 10 no longer claim an empty result means clean. (`src/agent/completionGate.ts`, `src/agent/tools/diagnostics.ts`, `src/webview/handlers/basePrompt.ts`)

- **Remembered instructions could be destroyed by a single transient read error.** `DurableMemoryStore.load()` caught every failure into `entries = []` and reported the store ready, so an unreadable or truncated file was indistinguishable from a first run. The store then persists the whole collection, so the next `addAll()` — which fires automatically on compaction, without the user doing anything — wrote `[]` over the file. Every rule the user had ever latched was gone, behind a log line that had scrolled past. Non-atomic writes closed the loop: `writeFile` straight over the live path leaves a truncated file if the process dies mid-write, which is exactly the input that triggers the destructive read next start. Both memory stores now share `jsonStore.ts`: an absent file is a clean empty load, but a store that exists and cannot be read is **never overwritten** — its bytes are moved aside to `<file>.unreadable-<ts>` and the failure is returned to the caller. If the bytes can be neither read nor moved, persistence refuses outright rather than destroy them. Writes go to a temp file and rename into place, so a crash leaves either the old file or the new one, never half of one. (`src/agent/memory/jsonStore.ts`, `durableMemory.ts`, `agentMemory.ts`)

- **Agent memories could silently fail to persist.** `AgentMemory.save()` and `.load()` swallowed every error into `logger.warn`, and auto-saves are fire-and-forget, so a failed write was indistinguishable from a successful one — the failure mode that made a CI run look flaky when it was really reporting lost data. Failures now surface as observable state (`getSaveError()`, `getLoadFailure()`) and log at error level; the auto-save chain catches at the chain level so one failure cannot poison every later save.

- **Concurrent saves to the same store raced and lost one of the writes.** The first cut of the atomic write used a shared `<file>.tmp`, so an auto-save overlapping a direct `save()` had both writing one temp path, the first renaming it away and the second failing with ENOENT. Temp paths are now unique per write. (Found by the new adverse-I/O tests, against my own first implementation.)

- **edit_file could not edit a CRLF file on any multi-line search.** The match was `text.includes(search)` over raw bytes with no line-ending handling anywhere in `src/agent/`. A model emits LF; a CRLF file holds `a\r\nb`; every multi-line search missed and the model was told "search string not found" — true, and useless, because the text was right there. Single-line searches worked, since the `\r` sits past the end of the search, so the failure looked arbitrary rather than systematic. Matching now runs through a new tiered matcher (`editMatch.ts`): byte-exact first, then line endings (CRLF/LF/CR), then trailing whitespace, then indentation. Every tier resolves to a real byte span in the original text, so uniqueness, token-boundary, duplicated-tail and syntax guards all still run against real file bytes, and each tier is tried only when the previous finds nothing — an exact match is never overridden, and a search that is unique byte-for-byte is never called ambiguous because a laxer tier found siblings. Tolerance is disclosed in the result (`[note: your search used LF line endings and <path> uses CRLF …]`) so the model's next search is right rather than merely rescued. (`src/agent/tools/editMatch.ts`, `fs.ts`)

- **A successful edit left mixed line endings behind.** The replacement was spliced in exactly as the model sent it — LF — so every edit to a CRLF file corrupted that file's convention line by line. Replacement text is now converted to the file's dominant ending on every path, including the fuzzy inferred-apply. Only the replaced span is rewritten: a genuinely mixed file keeps its untouched lines byte-identical rather than being silently normalized into a whole-file diff.

- **Text copied out of `read_file(mode='compact')` could never match the file it came from.** Compact right-trims every line (`line.trimEnd()`), so a model reading compact and searching with what it read missed on any line with trailing whitespace. The trailing-space tier absorbs it and the note points at the cause.

- **A retyped block lost its indentation and the edit failed.** The indent tier matches on content and re-indents the replacement onto the matched region's indentation, preserving relative nesting (a shift, not a re-tabulation — converting indent *width* would mean inferring the file's indent unit and gets Python wrong in more ways than it fixes). Disclosed with an explicit "verify the result", since this is the one tier making a judgment call.
- Two more `String.prototype.replace` string-form calls in the fuzzy inference path expanded `$&`/`$1` from model text; both now use the callback form, and the inferred-apply splices by offset.
- **edit_file had three implementations and only one of them was the good one.** The disk path carries the whole guard stack — already-applied recognition, tiered fuzzy recovery, uniqueness, token-boundary, duplicated-tail repair, syntax gate. Audit mode carried a partial copy, and review mode a hand-rolled `base.includes(search)` / `base.replace(search, replace)` pair with none of it. That review-mode copy used the STRING form of `String.prototype.replace`, so `$&`, `` $` ``, `$'` and `$n` in the replacement text were expanded as regex references and silently corrupted the queued file; it replaced only the first of N matches with no ambiguity check; and it answered a miss with a bare `Error: Search text not found in <path>` that handed the model nothing to recover from — 16 of 40 edit_file failures in the local audit log, including a five-call retry loop against `executeToolUses.ts` where the model reissued blind because the error said nothing. All three modes now route through one exported core, `resolveEditedText`, which decides what the file should become; each mode only chooses where to persist it. A guard added once protects disk, audit buffer and review queue together. Audit mode gains the token-boundary, duplicated-tail and syntax guards it never had — it previously accepted a signature-to-return-type replacement that left `string\n  return 42;\n}` in the buffer and reported success. (`src/agent/tools/fs.ts`, `src/agent/executor/reviewModeHandler.ts`, `src/agent/executor.ts`)
- Dead branch in the repeated-failure path: a second, byte-identical `looseTarget` suggestion sat behind the throw that always preceded it.
### Changed

- **The outcome-visibility diff now reaches every persistence mode.** `editFile.resultDiffChars` appended the bounded "What changed (verify this is what you intended)" diff to disk-path results only; audit-buffered and review-queued edits still returned a bare success line, so a model working under review or audit could not self-verify its own edit. Both now carry the same capped diff. The flag's default remains 0 (off) pending its A/B.
...
- **The injection screen ran at the wrong boundary and blinded retrieval on first-party source.** `indexSymbol` refused to store any symbol whose body matched the prompt-injection heuristic. But this index holds only vectors and metadata — never the body, which `semanticRetriever` re-reads from disk at render time — so dropping a row removed the symbol from semantic search while `read_file`, grep, the doc index and graph-walk expansion all still surfaced the same lines unfenced. It bought no safety and cost real code: on this repo it hid `registerDurableMemoryView`, `clearCmd`, `BedrockBackend` and `initMcpServer`. The screen now runs where the body is actually assembled for the prompt, fencing flagged content as untrusted data via `neutralizeInjections` instead of hiding the symbol — and it covers two paths the index-time check never could: graph-expanded hits, which never pass through the index, and the file-level retrieval route, which emitted whole file contents with no injection check at all. (`src/config/symbolEmbeddingIndex.ts`, `src/agent/retrieval/semanticRetriever.ts`)

- **Same-named symbols in one file silently overwrote each other.** `makeSymbolId` was `filePath::qualifiedName`, so 862 of 6,600 graph symbols (13%) collapsed onto an earlier row — 72 locals named `tool` in one test file became a single entry, last writer winning. IDs now carry a same-name ordinal. Line numbers would have disambiguated too, but they'd change the ID of every symbol below any inserted line, re-embedding most of a file on each save and orphaning the old rows; an ordinal only moves when same-named siblings are reordered. Ordinal 0 renders as the bare `filePath::qualifiedName`, so every already-stored ID stays byte-identical and nothing re-embeds. Assignment is centralized in `assignOrdinals` and consumed by all three ID producers — the live-edit path, the activation replay, and `SymbolIndexer.liveSymbolIds()` — because two of them disagreeing by one ordinal would make reconcile delete live rows that the next replay re-embeds, every start, forever. (`src/config/symbolEmbeddingIndex.ts`, `symbolIndexer.ts`)

- **Reconciliation is keyed on symbol ID, not file path.** `updateFile` re-indexes a changed file's current symbols but never removes the previous version's rows, and only whole-file deletes call `removeFile` — so a symbol renamed or deleted *within* a surviving file leaked forever, a second and slower instance of the same defect. `reconcile(liveSymbolIds)` replaces `reconcileFiles`, and mirrors removals into the Merkle tree per leaf rather than per file, since a file can lose one symbol and keep the rest. On this repo the first sweep drops 182 rows the graph no longer justifies — rows whose line ranges the retriever would otherwise slice blind from the current file — and the ordinal change admits ~1,363 previously unreachable ones. (`src/config/symbolEmbeddingIndex.ts`, `src/activation/workspaceIndexer.ts`)
...
- Reconciliation gates: orphan drop with live-set preservation, symbol-deleted-from-a-live-file, no-op on a fully-live set, pending-queue sweep, per-leaf Merkle mirroring, and a pre-ordinal-metadata row surviving the sweep (getting that wrong would delete and re-embed the entire index) — plus a 2,500-entry chunk-boundary replay test so an off-by-one in the yield loop can't silently truncate a large index.
- **`liveSymbolIds()` agrees exactly with the IDs the replay queues.** The load-bearing invariant behind ID-keyed reconciliation, asserted directly rather than trusted: the two sides are pinned to the same `cappedSymbols` + `assignOrdinals` pair, and the test fails if they ever diverge.
- Render-time injection gates: a flagged symbol body is fenced and still surfaces, a clean body is left byte-identical, and the file-level route fences too.

### Stats


- 8318 total tests (447 test files)
- 87 built-in tools, 11 skills
...
- **Remembered Instructions management surface** — a sidebar tree view listing every persisted instruction (reinforcement count shown), with per-entry forget, clear-all (confirmed), and refresh commands (`sidecar.memory.forgetInstruction` / `clearInstructions` / `refreshInstructions`). Refresh re-reads from disk because MCP-server sessions write the same store file. The store is now observable and correctable instead of a write-only JSON file. (`src/views/durableMemoryView.ts`)

- **Explicit supersession + conflict surfacing.** An instruction carrying the user's own update marker ("actually…", "instead…", "correction…", "change that rule…", "no longer…") replaces the existing entry it targets (token-overlap targeting, floor 0.3) and is announced as `📌 Updated remembered rule: "…" replaces: "…"`. A marker-less new rule that heavily overlaps an existing entry coexists and is flagged `⚠️ May conflict with an earlier rule` — because lexical similarity cannot distinguish "even→odd" (a replacement) from "even vs positive" (two compatible rules), similarity is never allowed to decide deletion; only explicit user language is. Proven on a three-session eval case (rule → update → application): seven discriminating pairs across haiku (ceiling), gemma4 ×2, qwen3.5 ×2 — every on-arm ended with exactly one store entry and applied the updated rule; every off-arm applied nothing or the stale rule. (`src/agent/memory/durableMemory.ts`, `src/agent/loop/compression.ts`)

- **Scenario-diverse cross-session cases + capability map.** Cross-session memory evidence now spans three shapes — numeric rule, style prohibition, project fact — instead of one. Honest map from the campaigns: numeric-rule and fact recall generalize down to gemma4 (2/2 pairs each); the style prohibition is ceiling-only — gemma4 reads the injected rule and violates it while generating anyway, a negation-compliance capability boundary of the model class, recorded as such rather than patched around (entries stay the user's verbatim words). (`tests/llm-eval/longHorizonCases.ts`)

- **Multi-session eval harness** — `sessionBoundariesAfterTurns` (arbitrary boundary sets, for update-then-apply cases) and `midSeedsAfterTurns` (per-session forcing mass at distinct corpus offsets), with mechanism counters carried across all boundaries. (`tests/llm-eval/longHorizonHarness.ts`)

- **Directive `write_file({})` error.** qwen2.5-coder's observed empty-args call now gets told exactly what to send — both fields, the complete file text, and that code printed in chat was never saved — instead of a schema dump that doesn't reroute weak models. (`src/agent/executor.ts`)

- **A hypothesis tested and rejected, recorded for honesty:** qwen2.5-coder's chat template demands `<tool_call>` JSON "with NO other text" while this prompt teaches with backtick `tool(arg=…)` snippets — plausibly the root of the code-as-text behavior v0.120's recovery rescues. A notation-vs-emission prompt note was built, flag-gated, and A/B'd: 15 pairs on the calculator session, base 8/15 vs note 6/15, 10 discordant pairs splitting 4–6 against, p=0.75, reprompt counters identical per turn survived in both arms. A powered null with a negative lean — the flag was deleted per the prove-or-prune discipline rather than shipped as unproven surface.

### Fixed


- **Reworded duplicates killed at the source.** The persist hook now stores only direct-latch instructions — verbatim-guaranteed extractions from the user's own messages — never the summary-section re-harvest, whose LLM-carried bullets could rephrase a rule enough to defeat content-addressing and accumulate near-duplicates. Store hashing is additionally normalized (case/punctuation-insensitive), so duplicates are impossible by construction rather than filtered after the fact. (`src/agent/loop/compression.ts`, `src/agent/memory/durableMemory.ts`)

- **Remembered-section authority framing.** At ceiling, the model quoted a remembered entry verbatim while claiming it had "no access to earlier sessions" — the injected section read as data about memory, not as memory. The section header now states the entries ARE the record of earlier sessions and are to be used directly. (`src/agent/memory/durableMemory.ts`)

- **Un-seeded sessions silently lost late instructions (instrument).** The supersession case's middle session carried no seed mass, never reached the compaction threshold, and its "actually… odd" update never persisted — session 3 then correctly applied the stale rule. Caught at ceiling before any local run (the counters made it a two-minute diagnosis: `memEntries=1` but the surviving entry was the original). Every session that must persist something needs its own forcing mass — hence `midSeedsAfterTurns`. (`tests/llm-eval/longHorizonHarness.ts`)

- **v0.121 stores are migrated on load.** The duplicate-kill changed the content-hash scheme to normalized text, but entries written by v0.121 keep their old-scheme IDs on disk — a rule re-latched under v0.122 would miss its own entry and create exactly the duplicate the scheme exists to prevent. `load()` now recomputes every ID and merges collisions (reinforcement summed, most-recent text kept), writing the migrated store back once. (`src/agent/memory/durableMemory.ts`)
...
Durable context — standing instructions now survive compaction and survive the session, both proven with unanimous paired evidence and confirmed live in the running extension. A rule stated once ("every numeric config value you write must be even") is deterministically latched when compaction fires, announced in chat ("📌 Remembered for future sessions"), persisted verbatim to `.sidecar/memory/`, and re-injected — injection-screened — into later sessions, where the model applies it correctly with the scope the user's own words state. The road there rebuilt the long-horizon instrument from the ground up (eight defects found and fixed, including a token budget that had never been wired — retroactively invalidating every prior "compaction" label in the eval history) and caught two live production 400s in the cloud-backend tool-pairing schema plus four gaps in the MCP server surface, all invisible to the synthetic apparatus and all found by ceiling-first validation and a real end-to-end run through the dev host.

### Added


- **Durable instructions through compaction — DEFAULT-ON** (`sidecar.compaction.durableInstructions`). A deterministic latch pulls instruction-shaped user sentences ("always/never/must/remember/from now on…") verbatim into a leading `## Standing instructions` summary section at compaction; the LLM path carries the section under a never-invent rule; re-latching carries entries through arbitrarily many compaction rounds. Proven: gemma4 paired A/B on memory-recall, off 0/8 vs on 8/8, McNemar 0–8 discordant, p=0.0078 — every pair flipped; ceiling-tier root cause first (even a frontier summarizer drops constraints through the old four-section prompt — no section owned them). (`src/agent/conversationSummarizer.ts`, `src/agent/loop/compression.ts`)

- **Cross-session instruction memory — DEFAULT-ON** (`sidecar.memory.persistInstructions`). Instructions latched at compaction persist to `.sidecar/memory/durable-instructions.json` (content-addressed, reinforcement-ranked, capped at 20, per-user gitignored state) and later sessions re-inject them as a provenance-labeled `## Remembered Instructions` system-prompt section — every entry passes injection screening at render, and each NEW entry is announced once in chat. Write path is the compaction hook by design (voluntary tool adoption measures ~0 on local models); no tool lets the model author arbitrary entries. Proven: gemma4 paired A/B on the new session-boundary harness, off 0/7 vs on 7/7 clean pairs, p=0.0156; confirmed end-to-end in the live extension via the MCP surface. SECURITY.md documents the trust boundary and residual risk. (`src/agent/memory/durableMemory.ts`, `src/webview/handlers/systemPrompt.ts`)

- **Session-boundary eval harness** — `sessionBoundaryAfterTurn` discards history mid-case so recall must travel through the memory store exactly as in production; mechanism counters (`splices=`, `latched=`, `memEntries=`) carried across the boundary; mid-conversation seed injection (`midSeedAfterTurn`) forces real summarization model-independently after the constraint is stated. New `lh-cross-session-recall` case. (`tests/llm-eval/longHorizonHarness.ts`, `tests/llm-eval/longHorizonCases.ts`)

### Fixed


- **Two live cloud-backend 400s: the tool-pairing schema, both directions.** A dangling `tool_use` (loop aborted mid-call, then history threading buried it) and its mirror — an orphaned `tool_result` whose assistant half was summarized away by compaction — each produced opaque request rejections on Anthropic/Bedrock while Ollama tolerated the shape silently. `repairDanglingToolUses` now enforces the pairing invariant symmetrically and idempotently at the request boundary: missing results are synthesized with an "interrupted" marker, orphaned results are downgraded to text blocks, both disclosed via warning so upstream sources stay visible. (`src/ollama/anthropicBackend.ts`, `src/ollama/bedrockBackend.ts`)

- **The MCP server surface silently bypassed the agent's context machinery.** `run_agent_task` spawned a bare loop: no durable-memory store (tasks neither recalled nor persisted instructions), no token budget (the compression default of 100k meant MCP tasks essentially never compacted), and a `SidecarDir.getPath`-before-`initialize` crash inside a void-ed promise meant enabling `sidecar.mcpServer.enabled` could produce no server and no error. All three wired/fixed; each `run_agent_task` call is its own session with full memory parity. (`src/mcpServer/agentServer.ts`, `src/activation/mcpServerSetup.ts`)

- **The long-horizon eval harness measured compaction that never happened.** The loop's compression budget reads only `AgentOptions.maxTokens` — never wired by the harness, so the threshold sat at 70k tokens and real summarization never fired in any long-horizon eval to date; "compaction" was tool-result truncation plus a token-drop heuristic. Also fixed en route: vacuity now keys on actual summary splices; the memory-recall case's trap prompt (it punished a defensible reading of an ambiguous rule) rewritten with an application-scoped contract; swallowed harness exceptions now surface their reason; budget-exhausted runs classify as `progressing`, not `diverged`. (`tests/llm-eval/longHorizonHarness.ts`, `tests/llm-eval/longHorizonCases.ts`)

- **Standing instructions died at the second compaction.** The summary carrier starts with `[Earlier conversation summary…]`, so the latch's synthetic-injection skip masked its own prior output — same self-masking class as the v0.120 action-reprompt bug, mirrored. The latch now re-harvests section bullets from prior summaries before the skip. (`src/agent/conversationSummarizer.ts`)

### Stats


- 8269 total tests (443 test files)
- 87 built-in tools, 11 skills

## [0.120.0] - 2026-07-23

Code-as-text recovery — the release where a scaffold earned default-on through the instrument instead of plausibility. Five paired campaigns (~250 runs) drove every change: the recovery package took qwen2.5-coder:7b from 0/28 to 11/27 on the multi-turn calculator session (McNemar 0–11 discordant, p=0.001 — the first result in this codebase to clear the ≥6-discordant honesty gate), a five-model validation found no regression anywhere (the machinery is provably dormant on capable models), and a budget-unbound re-run separated "can't ever reach the solution" from "reaches it slower than the eval budget" — restoring qwen3.5 and ministral from 0-for to solid passers with no code change and making that distinction (`converged`/`progressing`/`stuck`/`diverged`) first-class harness output. Two of the campaigns' failures were our own layers, not the model: an action reprompt that disarmed itself after one firing, and a parameter remap that rewrote perfect V2 insert calls into a rejected shape. Meta-lesson, again: read trajectories first — and validate every new calling convention through the full dispatch path the model's call takes.

### Added


- **Code-as-text recovery package is DEFAULT-ON** (`sidecar.recovery.codeAsText`) — deterministic recovery for models that do the work in chat text instead of tool calls, every layer a rescue path that stays dormant unless its failure shape occurs, each mined from a live failing trajectory: targeted reprompt wording when a turn contains an edit-shaped code fence or a fabricated `<tool_output>`/`<tool_response>` wrapper ("that code was NOT saved — call write_file(path=…, content=<complete file>)"); call-expression parsing (`write_file(path="x", content="…")` emitted as prose — kwarg-form, known tools only, rescue priority, triple-quoted Python strings, fences whose body opens with a tool call); literal-escape decode for content serialized as `\n`-escaped strings, applied only when the parse gate proves the decode fixed the file; fence-to-write synthesis when the model printed the complete target file and called nothing (single unambiguous target, language/extension agreement, self-import and test-target coherence guards, ≤2/run, loud disclosure); fused-anchor split for `insert_after` values that concatenate the existing anchor with the new code. Proven: 3-arm paired campaign, bare 0/28 vs recovery 11/27, p=0.001; five-model validation with zero net-negative discordants (gemma4 later showed a 0–3 uncontradicted lift once budgets were honest; granite posted its first-ever pass with the synthesizer firing 14×). (`src/agent/loop/textParsing.ts`, `src/agent/loop/actionReprompt.ts`, `src/agent/tools/syntaxCheck.ts`, `src/agent/tools/fs.ts`, `src/agent/loop/streamTurn.ts`)

- **Failure-mode verdicts in the long-horizon harness** — every run reports `converged` / `progressing` (budget artifact — empty or cut-off failing turn) / `stuck` (≥3 same-tool errors, the repeat-thrash signature) / `diverged`, because PASS/FAIL at a fixed budget conflates "can't reach the solution" with "reaches it slower than the clock": under an honest budget qwen3.5 went 0/5→8/10 and ministral 0/10→5/10 with no code change, their passes routinely running past the old 480s cap. Plus mechanism-fired counters (`steer=`, `reprompt=`) in the summary line so a null A/B can never hide a dead arm. (`tests/llm-eval/longHorizonHarness.ts`, `tests/llm-eval/longHorizon.eval.ts`)

- **Whole-file-rewrite edit strategy** (`sidecar.editStrategy.wholeFileRewrite`, default off) — system prompt directs the model to modify small files via read_file → write_file(<complete file>) instead of edit_file; the isolate-rewrite nudge stands down when on. Powered null on top of recovery (27 pairs, 6–7 discordant): measurably adds nothing at 3–5× the wall-clock, kept for manual use. (`src/webview/handlers/basePrompt.ts`, `src/agent/loop/isolateRewrite.ts`)

- **Insert-API V2** (`sidecar.editFile.insertApiV2`, default off) — the edit_file schema variant whose field names survive the naive English reading: `insert_after`/`insert_before` hold the EXISTING anchor ("insert after THIS"), `new_text` holds the code to add. The executor accepts both conventions unconditionally via deterministic disambiguation; the flag only swaps which convention the schema and prompt examples teach (moved in lockstep through production and both eval harnesses). Motivation: gemma4 read the V1 field names as positions in every campaign run — anchor in `insert_after`, payload nowhere. Functionally proven end-to-end (zero insert-confusion errors in the taught arm, V2 calls land); case-level power pending gemma4's day-to-day variance. (`src/agent/tools/fs.ts`, `src/agent/tools.ts`, `src/webview/handlers/basePrompt.ts`)

- **Directive edit_file errors for the observed insert misreadings** — `search: ""` no longer falls into "appears 69 times, add more surrounding context" (an empty string "appears" at every character); it names the two calls that ADD code. An `insert_after` value that already exists verbatim in the file gets "you gave the position, but not the new code" with the corrected call — under V2 teaching the same emission is half right and the error asks for exactly the missing field. (`src/agent/tools/fs.ts`)

### Fixed


- **The action reprompt disarmed itself after one firing.** Its own injected message didn't follow the `[`-prefix convention that `lastUserMessageText` skips, so after the first reprompt the "user's last message" WAS the reprompt — matching neither trigger — exactly when the model doubled down on fabricated `<tool_output>` roleplay. Both injected texts now carry the prefix; a second firing works. (`src/agent/loop/actionReprompt.ts`)

- **paramRemap rewrote perfect V2 insert calls into a rejected shape.** `new_text` is a Claude-style synonym for `replace`, so the remap turned `insert_after=<anchor>, new_text=<code>` into `replace`+`insert_after` — mutually exclusive, bounced 100% of the time. Caught three runs into the V2 A/B by the new `mode=stuck` verdicts. `new_text` alongside an insert field is canonical, never a synonym; without one the Claude-style recovery still applies. (`src/agent/executor/paramRemap.ts`)

- **`coerceArgValue` decodes double-quoted escape sequences.** `content="def add(a, b):\n  return a + b"` now lands with real newlines instead of literal backslash-n (JSON decode with verbatim fallback); Python triple-quoted literals unwrap raw. (`src/agent/loop/textParsing.ts`)

### Stats


- 8233 total tests (442 test files)
- 87 built-in tools, 11 skills
...
- **Example-replay guard** (scaffold 3.0.0, always on) — the executor bounces any tool call whose arguments verbatim-match the example embedded in that tool's own description (examples with ≥2 arguments only, so a legitimate single-key call can never collide by coincidence), before the review/approval gates. Motivating live failure: a bare "hi" sent llama3.2 replaying `ask_user`'s auth-flow example character-for-character, surfacing a fabricated question; the guard-probe sweep reproduced the family (`edit_file` example replayed on "thanks, great work!"). The catalog's 71 concrete examples stay — small models need the format demonstration — and a catalog-wide ratchet test fails CI if rewording an example would silently disarm the guard. Mutation-tested moat module. (`src/agent/executor/exampleReplayGuard.ts`)

- **Escalating dispatch-bounce messages** — schema-validation, malformed-JSON, example-replay, and unknown-tool bounces now escalate on consecutive identical repeats (2nd: "do not resubmit — change the arguments or approach"; 3rd+: "stop retrying; use a different tool or reply in plain text") and reset on any successful call of the tool, so transient stumbles amid progress never escalate. (`src/agent/executor/bounceEscalation.ts`)

- **Guard-candidate probe layer** — evidence-first guard development: provocation eval cases (no-signal turns, indirect file references, foreign-catalog phrasing) plus a five-signature trajectory scanner tally would-be executor guards without failing cases. First sweep (5 models × 4 config arms × 5 probes) confirmed the example-replay guard and ruled out three speculative guards (placeholder args, wrapper keys, name normalization: 0 firings in 100 cases, matching zero hits in 1,403 audit-logged dogfood calls). Plus `SIDECAR_EVAL_TRAJECTORY_DIR` (full trajectory JSONL dumps, stamped with model + config arm) and `SIDECAR_EVAL_CONFIG_OVERRIDES` (feature-flag sweeps across any eval file). (`tests/llm-eval/guardProbe.eval.ts`, `tests/llm-eval/guardCandidateScan.ts`)

- **Multi-trial eval reliability** — `SIDECAR_EVAL_TRIALS=N` runs each agent-loop case N times and reports pass@1, binary-entropy flakiness, and a PASS/FLAKY/FAIL verdict; only all-trials-failed is a vitest failure. For local models flakiness is the dominant failure mode (2/3 passing is pass@3≈96% but pass^3≈30%). `npm run eval:reliability` = 3-trial smoke set. (`tests/llm-eval/reliabilityMetrics.ts`)

- **Multi-turn latch eval cases** — regression cases for the observed "model latches onto previous prompts" failure: seeded prior exchanges (`setupMessages` + `setupMessagesRequired`) followed by a fresh turn, with a new `toolCallNotMatches` predicate for "no call whose arguments reference the previous task's file". Baseline on llama3.2: hard latch on gratitude turns (0/3 — redoes the previous edit every time), clean on topic-switch and instruction-bleed. (`tests/llm-eval/multiTurnCases.ts`)

- **`read_file` line ranges** — `start_line`/`end_line` (1-based, inclusive) now actually work. The base prompt told the model four times to read a range to grab an `edit_file` anchor, but the schema declared only `path`/`mode` and the executor ignored the range args — a model following the prompt asked for three lines and silently got the whole file, so the "read only the lines you need" guidance was unimplemented. The range returns raw lines with NO line-number prefix (copies verbatim into a `search`), takes precedence over `mode`, and reports out-of-range/inverted inputs. (`src/agent/tools/fs.ts`)

- **`edit_file` → `write_file` steer** (`sidecar.editFile.steerToWrite`, default off) — mirror of the write→edit escalation: after N failed `edit_file` calls on a file, redirect the model to rewrite the whole file with `write_file`. Motivating trajectory: weak models asked to ADD code to an existing file echo its existing functions into the insert fields (never producing the delta) and thrash to the iteration cap — but author the whole file cleanly via `write_file` (gemma4 probe 3/3). Ships opt-in: unit-tested and mechanically verified (gemma4/llama3.2 obey it), pass-rate benefit unproven. (`src/agent/loop/circularRewrite.ts`)

- **`edit_file` outcome-visibility diff** (`sidecar.editFile.resultDiffChars`, default 0/off) — optionally appends a bounded diff of the change to a successful `edit_file` result so the model can SEE what its edit did. (`src/agent/tools/fs.ts`)

### Fixed


- **Silently dropped text-form tool calls from llama3.2-class models.** The bare-JSON text-parsing path only read `name` at the top level, so the OpenAI function-call shape (`{"type":"function","function":{"name":…,"parameters":…}}`) never dispatched — and a truncated emission missing its final closing brace was skipped entirely instead of salvaged. Both observed live in the latch-case baseline: the model looked like it "chose" not to read a file when its call was actually lost in parsing. Nested shapes now parse; truncated calls emit the malformed-input marker so constrained repair gets a shot. Scaffold version bumped to 3.0.0 — weak-model pass rates are not comparable across this boundary. (`src/agent/loop/textParsing.ts`, `src/agent/scaffoldVersion.ts`)
...
- **Ollama models are no longer pinned in RAM forever.** `keep_alive: -1` (a May latency optimization) kept every model a session ever chatted with resident until Ollama restarted — a model-switching session accumulated ~17GB of permanent pins and took the host machine down (found in v0.119 host dogfood). Now `30m`, refreshed by every request: active sessions never pay the cold-reload penalty, switched-away models expire. (`src/ollama/ollamaBackend.ts`)

- **Per-model context caps.** KV-cache cost varies enormously by architecture: llama3.2 (global attention every layer) at the 131K default allocated a 17.4GB llama-server for a 2GB model. llama3.2 now defaults to a 64K window; gemma4 and others keep 131K; `sidecar.ollama.numCtx` still overrides everything. (`src/config/constants.ts`, `src/ollama/ollamaBackend.ts`)

- **The chat shows indexing progress.** A multi-minute activation (cold index or big post-churn reindex) previously showed nothing in the chat — it looked broken. A spinner banner now reports "Indexing N changed symbols…" until the embed queue drains, and the PKI status-bar spinner shows on warm-cache replays too, not just first runs. (`src/activation/workspaceIndexer.ts`, `media/chat.js`)
...
- **`edit_file` insert API garbled weak models via a prompt/schema contradiction.** The base prompt's "add a function" example taught `replace=<text + new function>` (restate the anchor), directly contradicting the `edit_file` schema's "use `insert_after` with ONLY the new text — do not restate the anchor" — and the insert API was never demonstrated in any example. gemma4 split the difference: it echoed the existing function into `search` + `insert_before` + `insert_after` at once, never generated the new code, and thrashed to the iteration cap. The example is now aligned to `insert_after` with an explicit "adding vs. replacing — pick the right field" rule, and the executor rejects the anchor-in-insert inversion and the both-insert-fields shape instead of silently duplicating the anchor and reporting success. A base-prompt audit confirmed no other tool example contradicts its schema. (`src/webview/handlers/basePrompt.ts`, `src/agent/tools/fs.ts`)

- **The edit→write and write→edit steers no longer oscillate.** When a model's generated content was syntax-broken, neither tool could succeed, so the two mirror guards ping-ponged it (edit fails → "use write_file" → write rejected for the syntax error → "use edit_file" → repeat) until it gave up — observed on gemma4's calculator CLI turn. The corrective edit→write steer now outranks the advisory isolate-rewrite nudge for any file it deliberately redirected, until a later successful edit clears the flag. (`src/agent/loop/isolateRewrite.ts`)

### Stats


- 8162 total tests (441 test files)
- 87 built-in tools, 11 skills
...
- **Analytic-bound gate (§5 numerical vertical).** Completes the numerical vertical begun in 0.115: a kernel that _declares_ a value bound (`# bounds: 0 <= result <= 1`, `# invariant: energy >= 0`, `sum == 1`) but does not _enforce_ it is flagged, with the exact assertion to add — a comment is not a guarantee. Parses bound declarations, checks enforcement, and emits the assertion. Opt-in + advisory-first. (`src/agent/analyticBounds.ts`)

- **Property-based test synthesis (`synthesize_property_test`).** Emits a runnable Hypothesis test that tries to _violate_ a kernel's declared invariants (symmetry, idempotence, monotonicity, bounds) across random inputs — the third pillar of the §5 vertical, turning "tests pass" into "the stated properties hold." (`src/agent/tools/propertyTest.ts`)

- **Keep-best ratchet — Pareto-safe scaffolding (do-no-harm).** Snapshot → apply scaffold-driven changes → re-verify → revert on regression, so scaffolding can never turn a passing run into a failing one. Guards against the over-engineering the completion gate was observed to cause (emitting ~32 KB of test-churn where the bare model stayed ~450 b). Opt-in `sidecar.scaffolding.keepBest` (+ `keepBestOverEngineerBytes`). (`src/agent/loop/keepBestRatchet.ts`)
...
- **Mangled tool-call salvage.** Some runtimes (again Ollama's native qwen3.5 parser) intermittently emit a tool call whose NAME captured the entire call expression — `read_file(path="src/x.ts")` with empty arguments — so dispatch failed with an opaque "Unknown tool" and the model burned its whole iteration budget retrying. Tool dispatch now salvages a `name(...)` call-expression name: it resolves the base name and recovers the arguments from the parentheses (Python-style kwargs or an embedded JSON object). Real tool names are plain identifiers, so it never fires on a valid call. (`src/agent/loop/textParsing.ts` `parseMangledToolName`, `src/agent/executor.ts`)

- **Text tool-call arg extraction — accept the `args` key.** Models that emit tool calls as text JSON (rather than the native tool_calls field) sometimes use the shape `{"tool": "read_file", "args": {"path": "x"}}` — `devstral:24b` does this under the full agentic payload. `parseTextToolCalls` checked `arguments` / `parameters` / `input` but not `args`, so the arguments silently dropped to `{}`, the tool failed with "missing required parameter", and the model thrashed retrying. Added `args` to the argument-extraction chain across all text patterns (and `tool` to the `<tool_call>` name chain). (`src/agent/loop/textParsing.ts`)
