...
- **Project Knowledge Index — graph-walk retrieval enrichment** (v0.61 step b.4, fourth and final core step of the PKI feature arc). `project_knowledge_search` now walks the existing `SymbolGraph`'s `calls` edges outward from each direct vector hit and surfaces symbols whose text wouldn't have scored but whose structural relationship to a scored symbol is load-bearing — the canonical "where is auth handled?" example now returns both `requireAuth` (direct vector hit) AND every route handler that wraps it (reached via `getCallers`), with the call site resolved to its *containing* symbol so the output points at the enclosing function rather than a raw line number. New `enrichWithGraphWalk(directHits, graph, { maxDepth, maxGraphHits })` helper: BFS per starting hit, global budget cap on added symbols, dedup across frontier starts so two direct hits that share a caller don't double-count, scores decay as `directScore * 0.5^hops` so direct hits always outrank reached ones. New tool params: `graphWalkDepth` (default 1, clamped to [0, 3]) and `maxGraphHits` (default 10, clamped to [0, 50]); setting `graphWalkDepth: 0` opts out of graph walk entirely. Response header distinguishes the two cases: "Found N symbols" for vector-only vs. "Found N direct + M graph-reached symbols" when graph enrichment kicked in. Each hit's relationship column shows either `vector: 0.823` or `graph: called-by (1 hop from requireAuth)` so the model sees why each result surfaced. +8 tests covering direct-hits-unchanged paths (null graph, depth 0), caller surfacing, hop-score decay, budget cap, and dedup across frontier starts; +2 integration tests on the tool response shape (enriched header + relationship rendering, depth-0 opt-out).
- **Project Knowledge Index — `project_knowledge_search` tool** (v0.61 step b.3, third of the PKI feature arc). New agent tool [`project_knowledge_search`](src/agent/tools/projectKnowledge.ts) exposes the symbol-level semantic index to the model. Takes `query` (required), optional `maxHits` (default 10, clamped to 50), `kindFilter` (narrow to `["function", "class", …]`), and `pathPrefix` (restrict to a subdirectory). Returns one line per hit as `filePath:startLine-endLine\tkind\tqualifiedName\t(vector: 0.NNN)` — a shape that `read_file` + line-slicing can parse back directly so the model can jump to the exact range without a second search round-trip. Graceful degradation: returns a short "not enabled" hint when `sidecar.projectKnowledge.enabled` is off, a "warming up" hint while the embedding model is still loading, and a "no matches" hint with a `grep` fallback suggestion when the index is populated but nothing scored. New `ToolRuntime.symbolEmbeddings` field + `setSymbolEmbeddings()` exporter wire the index into the tool registry the same way `setSymbolGraph` wires the graph. +9 tests covering the "not wired" / "warming up" / "empty result" / "missing query" error paths, response formatting for single-line and multi-line symbols, `kindFilter` + `pathPrefix` forwarding, `maxHits` clamping to [1, 50], and empty-array `kindFilter` dropping (so a caller passing `[]` doesn't accidentally filter to zero kinds).
- **Project Knowledge Index — wire to indexing pipeline** (v0.61 step b.2, second of the PKI feature arc). Hooks `SymbolIndexer` into the new `SymbolEmbeddingIndex`: every file the graph parses now also feeds each extracted symbol's body through a debounced `queueSymbol` + `flushQueue` batch drain (500 ms window, 20 per batch) so a whole-workspace scan doesn't serialize on a single embed at a time. New `SymbolIndexer.setSymbolEmbeddings(index, maxSymbolsPerFile?)` attaches the embedder and a per-file safety cap (defense against auto-generated files with tens of thousands of declarations). `removeFileFromGraph` and the debounced delete path both drop the file from the embedder too, so renames and deletes don't leave orphan vectors. Extension activation wires the index behind a new `sidecar.projectKnowledge.enabled` setting (default `false` during the v0.61 build-out; flips to `true` once the full arc ships). New settings: `sidecar.projectKnowledge.enabled`, `sidecar.projectKnowledge.maxSymbolsPerFile` (default 500). Per-symbol embed errors log a warning but don't abort the batch — one bad symbol can't block the rest. +8 tests: 4 on the queue primitive (drain, coalesce-in-place on re-queue, removeFile drops queued entries, per-symbol error isolation) and 4 on `SymbolIndexer` wiring (null-embedder preserves pre-PKI path, parsed symbols feed into queue with body intact, removeFileFromGraph propagates, `maxSymbolsPerFile` cap honored).
- **Project Knowledge Index — symbol-level embedding primitive** (v0.61 step b.1, first of the PKI feature arc). New [`SymbolEmbeddingIndex`](src/config/symbolEmbeddingIndex.ts) class: symbol-granularity sibling of the existing file-level `EmbeddingIndex`. Same `@xenova/transformers` MiniLM pipeline, same 384-dim space (so queries can cross the two backends during migration), different unit of indexing. `indexSymbol({ filePath, qualifiedName, name, kind, startLine, endLine, body })` embeds the symbol's body (prefixed with `qualifiedName (kind)` for structural context) and stores it keyed by `filePath::qualifiedName`. Content-hash short-circuit means re-indexing the same body is a cheap no-op, so a file save that doesn't touch a particular function skips its re-embed. `search(query, topK, { kindFilter?, pathPrefix? })` returns structured `SymbolSearchResult[]` with per-hit kind/path/lines so downstream tool-call rendering can surface the exact function rather than the whole file. `removeSymbol(id)` + `removeFile(path)` hooks for the indexing pipeline that ships in b.2. Persists to `.sidecar/cache/symbol-embeddings.{bin,meta.json}` — separate files from the file-level index so both backends coexist during the migration. Stand-alone primitive with no workspace wiring yet — b.2 will wire file-change events to the `SymbolGraph` → extract bodies → `indexSymbol`. +19 tests: indexSymbol skips on hash match, re-embeds on body change, no-op when model unavailable, search respects topK / returns [] on empty index / filters by `kindFilter` + `pathPrefix`, and removeFile returns the dropped-symbol count.
- **Audit Mode — git-commit buffering** (v0.61 step a.4, fourth of the v0.60 Audit Mode Phase 2 deferrals). The `sidecar.audit.bufferGitCommits` setting (shipped inert in v0.60) now actually gates commit execution: when audit mode is active with the flag on, `git_commit` tool calls queue into the audit buffer via new `AuditBuffer.queueCommit(message, trailers?)` instead of running `GitCLI.commit` immediately. Queued commits execute in FIFO order as the last step of a flush that empties the buffer — so the user sees one atomic "accept everything" boundary covering file writes + the commit that references them. Subset flushes leave commits queued (agent's commit presumably covers files still pending). New `BufferedCommit` + `ExecuteCommitFn` types, `listCommits()` / `hasCommits` getters, and a `committed` field on `flush()`'s return so the UI can report "accepted N changes and ran M commits." Per-path `clear()` preserves commits; full `clear()` drops them. Persistence schema bumped to v2 — new envelope `{ entries, commits }` — with transparent v1→v2 migration on load (commits default to `[]`). Commit-failure semantics: file writes stay on disk (they already landed), failed commit throws `AuditFlushError`, unprocessed commits remain queued for retry. +14 tests: 10 on the buffer (queue + FIFO + full-vs-subset flush semantics + failure path + clear interactions) and 4 on the `git_commit` tool (audit mode routing on/off, `auditBufferGitCommits` flag on/off, trailers preserved).
- **Audit Mode — buffer persistence across reloads** (v0.61 step a.3, third of the v0.60 Audit Mode Phase 2 deferrals). Pending agent changes survive extension reloads, window reloads, and VS Code restarts. New [`AuditBufferPersistence`](src/agent/audit/auditBuffer.ts) interface (`save` / `load` / `clear`) hooks into `AuditBuffer.setPersistence()`; buffer mutations (`write` / `deleteFile` / `flush` / `clear`) automatically call through so the on-disk snapshot stays current. Concrete FS-backed implementation in [`auditBufferPersistence.ts`](src/agent/audit/auditBufferPersistence.ts) serializes the full buffer to `.sidecar/audit-buffer/state.json` via `workspace.fs`, including schema versioning (v1) so future format bumps can migrate cleanly. New `AuditBuffer.restore(entries)` bulk-loads previously-persisted state without re-triggering persistence. Extension activation wires persistence + checks for a prior-session buffer; if any entries are found, prompts the user with `Review` / `Discard` (ESC defaults to Review — nothing is silently re-staged). `.sidecar/audit-buffer/` added to `SidecarDir.SUBDIRS` + `.gitignore`. Best-effort semantics: `save` failures log a warning but never fail the mutation that triggered them (losing a few minutes of buffer to a disk-full is strictly less bad than refusing the agent's writes). 64 MB hard cap on persisted file size prevents runaway state. +12 tests covering roundtrip, empty-buffer returns null, schema-mismatch rejection, corrupted-JSON rejection, per-entry shape filtering, integration with every mutation path (`write` / `deleteFile` / `flush` → `clear` / `clear` / `restore`), and save-error swallowing.
- **Audit Mode — conflict detection on flush** (v0.61 step a.2, second of the v0.60 Audit Mode Phase 2 deferrals). Pre-flush pass reads current disk state for every entry about to flush and compares it against `entry.originalContent` (the baseline captured when the agent first buffered the write). If any entry's disk content has diverged — user hand-edited the file during review, another tool rewrote it, rebase happened — the user now sees a modal warning dialog via new `showConflictDialog(message)` method on `AuditReviewUi`, with the conflicting paths enumerated in the message body. `Apply Anyway` proceeds with the flush (agent's version wins); cancel aborts the flush and preserves the buffer so the user can reopen review to see the fresh state. Subset-aware: per-file accept only prompts on conflicts in that file, not the whole buffer. Closes the silent-stomp failure mode where a mid-review disk edit would vanish on accept. +6 tests covering single-file modified conflict, cancel preserves buffer, disk-matches-baseline skips dialog entirely, "deleted from disk" wording, multi-file enumeration, and subset scoping on per-file flush.
