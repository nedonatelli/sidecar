...
- **Audit Mode — Review commands** (v0.60 step d.2, part 2 of the Audit Mode feature). Three new user-facing commands in [`agent/audit/reviewCommands.ts`](src/agent/audit/reviewCommands.ts) close the Audit Mode loop: `SideCar: Audit: Review Buffered Changes` opens a QuickPick listing every pending file with op marker + size hint plus Accept All / Reject All bulk actions (picking a file opens VS Code's native diff editor showing the captured `originalContent` against the buffered new content); `SideCar: Audit: Accept All Buffered Changes` flushes every staged entry atomically via `workspace.fs` (parent directory creation on demand, `{ useTrash: true }` on deletes) and surfaces `AuditFlushError` as a user-facing error with rollback count on partial failure, preserving the buffer so the user can retry after fixing disk conditions; `SideCar: Audit: Reject All Buffered Changes` clears the buffer after a modal warning-dialog confirmation. Handlers are structured around a small `AuditReviewUi` abstraction so tests drive them through a fake shim — no `window.*` stubbing required. `createDefaultAuditReviewUi()` binds the shim to real VS Code APIs (`showQuickPick`, `showWarningMessage({ modal: true })`, `commands.executeCommand('vscode.diff', ...)`) in one place; extension.ts registers the commands as thin wrappers that defer to the module. +13 tests covering empty-buffer early returns for all three commands, pick-list shape (bulk actions + per-entry rows), dispatch from review → accept/reject/diff, flush through `workspace.fs.writeFile` + `workspace.fs.delete` with `useTrash`, `AuditFlushError` surfacing + buffer preservation, and reject-confirmation dismissal leaves the buffer intact.
- **Audit Mode — `AuditBuffer` primitive + fs.ts interception** (v0.60 step d.1, part 1 of the Audit Mode feature). New `sidecar.agentMode: 'audit'` tier in which every `write_file` / `edit_file` / `delete_file` tool call diverts to an in-memory [`AuditBuffer`](src/agent/audit/auditBuffer.ts) instead of touching disk. Read-through: `read_file` sees the buffered content for paths the agent has already written this session, so multi-step edits stack correctly without the agent needing any awareness of the buffer. Atomic `flush(writeDisk, deleteDisk, paths?)` applies every staged change at once; any per-write failure rolls back the previously-applied entries and throws `AuditFlushError` with the applied/failed lists so the caller can surface the partial state. `clear(paths?)` drops entries without touching disk (reject path). Shell commands still run normally — the scope is the agent's explicit file-authoring surface, not every possible side effect, matching the practical threat model (`write_file` is the primary way hallucinations become persistent damage). New settings: `sidecar.audit.autoApproveReads` (default `true`), `sidecar.audit.bufferGitCommits` (default `true`; not yet wired — feature flag for d.2). +19 tests covering the create / modify / delete ops, create-then-delete collapse, read-through for modify / create / delete states, subset flush, atomic rollback on mid-flush failure, and clear semantics. **No UI surface yet** — the review commands (`SideCar: Review Audit Buffer` / Accept All / Reject All) ship as step d.2 next.
- **Regression Guard Hooks** (v0.60 step c, feature). Declarative shell-command guards in `sidecar.regressionGuards` act as hard gates the agent must pass before proceeding. Each entry declares a `name`, `command`, and `trigger` (one of `post-write` · `post-turn` · `pre-completion`); optional fields `blocking` (default `true`), `timeoutMs` (default `30000`), `scope` (glob filter — guard only fires when touched files match), `maxAttempts` (default `5` consecutive failures before giving up and emitting a one-time escalation message), and `workingDir`. Implemented as [`RegressionGuardHook`](src/agent/guards/regressionGuardHook.ts) — a `PolicyHook` that registers on the existing `HookBus` after the four built-in hooks, so every guard gets the same error-handling and ordering behavior as auto-fix / stub-validator / critic / completion-gate. When blocking and exit != 0, the guard's stdout + exit code are injected as a synthetic user message so the agent can read the error and revise on the next iteration. When non-blocking (e.g. a bundle-size budget where exceeding is a yellow flag), the output surfaces as a warning via `callbacks.onText` and the loop keeps going. Use cases the built-in lint/test suite can't express: physics invariants (`python verify_physics.py`), proof re-checks (`coq_check proofs/`), API contract diffs (`npx oasdiff breaking spec.yaml HEAD`). First-time workspace trust gate via `checkWorkspaceConfigTrust` — same contract as `hooks`, `mcpServers`, `customTools`, `scheduledTasks`. Global `sidecar.regressionGuards.mode` setting (`strict` / `warn` / `off`) toggles all guards off or into advisory mode without editing individual entries — useful for known-broken-refactor opt-outs. +24 tests covering `validateGuard` schema, post-write trigger filtering (runs only on mutating turns), post-turn unconditional fire, pre-completion via `onEmptyResponse`, scope glob matching, blocking vs non-blocking behavior, attempt-budget escalation + reset-on-success, and `buildRegressionGuardHooks` trust + mode handling.
- **Secret redaction in hook + custom-tool env vars** (v0.60 step b, audit cycle-3 MEDIUM #7). New `redactSecrets()` helper in [`securityScanner.ts`](src/agent/securityScanner.ts) replaces every match of the existing `SECRET_PATTERNS` with `[REDACTED:<name>]`. Called by [`executor.ts`](src/agent/executor.ts) before setting `SIDECAR_INPUT` / `SIDECAR_OUTPUT` on hook child-process environments, and by [`tools.ts`](src/agent/tools.ts) before forwarding user input to a `custom_*` tool's subprocess. Without this, a tool call whose input or output happened to contain an API key (e.g. after `read_file` on a `.env` that slipped past the sensitive-file guard, or an HTTP response with an Authorization header) would land verbatim in the child env, from which every subprocess the hook spawns inherits the secret. +10 tests covering empty input, secret-free passthrough, AWS keys, GitHub tokens, Anthropic keys in JSON-stringified payloads, database connection strings, multiple distinct secrets, repeat occurrences of the same secret, JWT tokens, and private-key headers.
