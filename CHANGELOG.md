...
- **Example-replay guard** (scaffold 3.0.0, always on) — the executor bounces any tool call whose arguments verbatim-match the example embedded in that tool's own description, before the review/approval gates. Motivating live failure: a bare "hi" sent llama3.2 replaying `ask_user`'s auth-flow example character-for-character, surfacing a fabricated question; the guard-probe sweep reproduced the family (`edit_file` example replayed on "thanks, great work!"). The catalog's 71 concrete examples stay — small models need the format demonstration — and a catalog-wide ratchet test fails CI if rewording an example would silently disarm the guard. Mutation-tested moat module. (`src/agent/executor/exampleReplayGuard.ts`)
