...
### v0.62 work-in-progress

- **Merkle fingerprint — query-time descent** (v0.62 step d.3, third and final of the Merkle arc). `SymbolEmbeddingIndex.search` now walks the Merkle tree's file-level aggregated vectors to pick candidate subtrees *before* scoring individual leaves — turns an O(total symbols) cosine scan into O(picked files × avg symbols per file). Candidate count is `max(10, topK × 3)` so filters + downstream graph walk have room without blowing past the full scan. Descent is active when a tree is wired AND populated; empty-tree edge case detects via `getFileNodeCount() === 0` and falls through to the full scan so the first-query moment after a fresh cache doesn't silently drop every hit. Extension activation now creates a `MerkleTree` + calls `setMerkleTree` when `sidecar.merkleIndex.enabled` (default `true`) is on and PKI is enabled — the two are architecturally coupled per the ROADMAP but kept on separate toggles so a user can debug retrieval regressions by disabling Merkle without losing PKI entirely. New Merkle-on RAG-eval parity test [`merkleParity.test.ts`](src/test/retrieval-eval/merkleParity.test.ts) re-runs every golden case with descent active and asserts the aggregate stays at-or-above the same ratchet floors the non-Merkle baseline hits. Current baseline with Merkle on the fixture: identical to without (meanP=0.492 / meanR=1.000 / meanF1=0.593 / meanRR=0.939) — expected, since the 8-file fixture has fewer files than the descent candidate count. Real pruning wins land on real workspaces with 100+ files. +8 tests: 2 on index-level integration (descent filters without breaking relevant-result surfacing, empty-tree fall-through) + 6 on the Merkle-parity eval (aggregate-metric floors + per-case scorecard emission + tree-wired sanity check).
- **Merkle fingerprint — keystroke-live updates** (v0.62 step d.2, second of the Merkle arc). Wires the `MerkleTree` primitive into `SymbolEmbeddingIndex` so tree state stays current with every index mutation — no separate subscription layer, no debounce surprises. New `setMerkleTree(tree)` hook attaches a tree and replays every persisted entry into it on attach (so the tree reflects prior-session state immediately, not after the next re-scan). Every `indexSymbol` call now computes a Merkle leaf hash (SHA-256 over `filePath|qualifiedName|kind|startLine-endLine|body`), stores it alongside the body MD5 in `SymbolMetadata`, and mirrors the leaf into the tree; `removeSymbol` / `removeFile` drop the matching leaves. The re-embed short-circuit now compares BOTH the body hash AND the Merkle hash — so a move-without-body-change (line range shifted from 10→12 by an edit above) skips the embed but still flips the tree fingerprint, matching the ROADMAP's "fingerprint changes iff symbol state changes" invariant. When the body hash matches but the Merkle hash doesn't, we reuse the stored vector via the new `VectorStore.getVector(id)` method rather than re-embedding — saves ~20 ms per "cosmetic move" on a typical edit. `flushQueue` fires `tree.rebuild()` after each batch drain (not per-symbol) so a whole-workspace scan rebuilds N file-nodes + the root O(1) times instead of O(N). New `getMerkleRoot()` accessor on the index surfaces the root hash for callers that need a single-value fingerprint (cache validity, cross-machine sync). `SymbolMetadata.merkleHash` is optional on the type for forward compat — pre-d.2 persisted caches work as before; entries without the field skip replay and populate lazily on next file re-index. +6 tests: setMerkleTree replays stored entries, addLeaf mirrors on indexSymbol, flushQueue triggers rebuild, remove paths propagate to tree, getMerkleRoot default empty, pre-d.2 cache replay-skip.
- **Merkle fingerprint — tree primitive** (v0.62 step d.1, first of the Merkle feature arc). New [`MerkleTree`](src/config/merkleTree.ts) class implements a content-addressed tree over symbol leaves, with aggregated embeddings at interior nodes for query-time subtree pruning. Structure: leaves are symbols, one interior node per file aggregates its symbols (hash + mean-pooled vector), and the root aggregates all file nodes into a single 32-char-hex fingerprint that changes iff any symbol in the workspace changed. Hash: SHA-256 on canonical `filePath|qualifiedName|kind|startLine-endLine|body` string (ROADMAP called for blake3 as default; we ship the `sha256` fallback now and add a blake3 adapter later — same backend-abstraction pattern as `VectorStore`). `addLeaf` / `removeLeaf` / `removeFile` mutations mark the affected file(s) dirty; `rebuild()` refreshes only dirty file-nodes + the root, so keystroke-live semantics are cheap to bolt on in d.2 (small number of affected files → O(log n) work). `descend(queryVec, k)` scores every file node's aggregated vector against the query and returns the top-k files' leaf IDs, ready to be fed to the leaf-level HNSW/flat scanner — this is the query-time pruning hook d.3 wires into `SymbolEmbeddingIndex`. Order-independent aggregation (child hashes sorted before concatenation) means two trees with the same leaves in different insertion orders produce the same root; same-leaves-different-order is an invariant the test suite pins. Cross-file moves (leaf changes `filePath`) correctly dirty both old and new files. Pure data structure — no disk I/O, no event wiring; persistence + live updates land in d.2. +27 tests: 9 on the pure hashing/pooling helpers (deterministic, order-independent, zero-mean safety) + 18 on the tree (CRUD, cross-file move, dimension enforcement, root hash determinism + reversibility + change propagation, file-node mean-pool, descend top-k + leafId ordering).
- **RAG-eval — LLM-judged metrics** (v0.62 step e.3, third of the v0.62 eval arc). Adds a new eval layer that asks a real frontier model to rate retrieval quality on two axes: per-hit *Faithfulness* (`RELEVANT` / `BORDERLINE` / `IRRELEVANT` verdict for each symbol, scored 1.0 / 0.5 / 0.0) and per-query *Answer Relevancy* (`ANSWERED` / `PARTIAL` / `MISSED` for the concatenated top-K). Runs under `npm run eval:llm`, not the main deterministic suite — the same split [`prompt.eval.ts`](tests/llm-eval/prompt.eval.ts) and [`agent.eval.ts`](tests/llm-eval/agent.eval.ts) use. Skips cleanly when no API key is set so forgetting `ANSWERED_API_KEY` gives a green run instead of a red one. Reuses the v0.62 e.1 synthetic fixture + golden cases so LLM-judged scores can be compared case-for-case against the deterministic metrics from e.2 — a divergence (e.g. deterministic precision says 1.0 but LLM Faithfulness says 0.3) flags a golden-set calibration issue. Architecture split: pure prompt-building + verdict parsing lives in [`src/test/retrieval-eval/judgeParsing.ts`](src/test/retrieval-eval/judgeParsing.ts) and runs in the main suite (prompt regressions surface before any tokens are spent); backend-aware judge functions live in [`tests/llm-eval/retrievalJudge.ts`](tests/llm-eval/retrievalJudge.ts) and fire only under the eval runner. Prompt caps: 2000 chars per-hit body, 10-hit hit cap on the answer judge with 600 chars per-hit body — total prompt stays bounded regardless of what the caller throws in. Unparseable judge responses (rate-limit, empty, chatty) score 0 so bad responses can't silently inflate the aggregate. +14 tests: 8 on verdict parsers (case-insensitive, whitespace-tolerant, unparseable-→0) + 6 on prompt builders (rubric text present, body truncation, hit cap).
- **RAG-eval — deterministic metrics + CI ratchet** (v0.62 step e.2, second of the v0.62 eval arc). New [`metrics.ts`](src/test/retrieval-eval/metrics.ts) implements the standard set-based IR metrics: `contextPrecisionAtK`, `contextRecallAtK`, `f1ScoreAtK`, `reciprocalRank`, plus `scoreQuery` / `aggregateScorecards` for per-query + macro-averaged aggregates. Set-based rather than RAGAs' rank-weighted variant because the set math is easier to triage in regression debugging and the golden dataset already pins which symbols are expected. New [`baseline.test.ts`](src/test/retrieval-eval/baseline.test.ts) runs every golden case through the metrics and asserts the aggregate stays at-or-above pinned floor thresholds — same ratchet pattern as `vitest.config.ts` coverage gates. Current baseline against the v0.62 e.1 fixture: `meanPrecisionAtK=0.49`, `meanRecallAtK=1.00`, `meanF1AtK=0.59`, `meanReciprocalRank=0.94`. Thresholds pinned ~4–5pp below so normal variance doesn't trip the ratchet; the test also logs per-case scorecards so a reviewer can see which cases drive aggregate movement when the floor shifts. Ratchet upward when retrieval quality improves (Merkle prune in d.3, reranker, etc.). +27 tests: 21 on the metric primitives (precision/recall/F1/RR edge cases, empty input, k-clamping, vacuous-recall, first-hit-wins semantics) + 6 on the aggregate ratchet (one per metric + scorecard emission + case-count invariant).
- **RAG-eval — golden dataset + harness** (v0.62 step e.1, first of the v0.62 eval arc). New deterministic retrieval-eval harness under [`src/test/retrieval-eval/`](src/test/retrieval-eval/) that runs as part of `npm test` (no LLM or network needed). Synthetic fixture in [`fixture.ts`](src/test/retrieval-eval/fixture.ts) — a miniature web service (auth middleware, route handlers, DB layer, utils, type declarations) with known-correct "where is X?" answers; hand-built so ground truth is explicit and tests stay portable across contributor forks. Harness in [`harness.ts`](src/test/retrieval-eval/harness.ts) wires a `SymbolEmbeddingIndex` + `SymbolGraph` populated from the fixture against a deterministic fake embedding pipeline (stable word-prefix → slot mapping shared across the run, so query vectors are reproducible). `runGoldenQuery` threads through the *real* production retrieval path — including `enrichWithGraphWalk` — so eval scoring tests the shipped code, not a parallel implementation. Golden-case dataset in [`goldenCases.ts`](src/test/retrieval-eval/goldenCases.ts): 11 cases covering concept search (auth middleware, token verification, DB ops, logging, formatting), graph walk (callers of `requireAuth` surface from an auth query), kind filters (interfaces-only / functions-only), and path prefix scoping (`src/routes/` / `src/utils/`). +16 tests: 11 golden-case assertions (every expected symbol appears in top-K) + 5 harness invariants (depth 0 emits zero graph-relationship hits, depth 1 does emit them, `kindFilter` drops non-matching kinds entirely, `pathPrefix` constrains to subtree, direct vector hits always outrank graph hits). Sets the baseline that e.2 formalizes into `ContextPrecision` / `ContextRecall` metrics and e.3 extends with LLM-judged `Faithfulness` / `AnswerRelevancy`.
- **PKI — backend abstraction scaffold** (v0.62 step c.2, second of the v0.61 PKI deferrals). Extracted the `SymbolEmbeddingIndex`'s vector storage into a pluggable [`VectorStore<M>`](src/config/vectorStore.ts) interface with a `FlatVectorStore<M>` implementation that matches v0.61 behavior exactly — same linear-cosine-scan semantics, same on-disk format (bit-for-bit compatible: the caller's `modelId` is preserved via `extraMeta`, so existing `.sidecar/cache/symbol-embeddings.{bin,meta.json}` keep working without a rebuild). Methods: `upsert`, `remove`, `removeWhere`, `search` (with optional metadata filter — flat filters in-memory, a future Lance backend pushes the predicate into a WHERE clause), `size`, `getMetadata`, `entries`, `persist`, `restore`, `clearPersisted`. Persist compacts orphan rows from prior `remove` calls so repeated edit-persist cycles don't bloat the bin file. `SymbolEmbeddingIndex` is now a thin domain layer (embedding pipeline, hash short-circuit, kind/path filter translation) that delegates storage to the store — constructor accepts an optional injected `VectorStore<SymbolMetadata>` so Lance (or any other backend) drops in without changes here. New `sidecar.projectKnowledge.backend: 'flat' | 'lance'` setting (default `flat`); selecting `lance` today logs a one-time warning + user toast and falls back to flat, reserving the name for a future release. LanceDB native-binding work is explicitly out of scope for v0.62 — it's a 2–3 day cross-platform project and gates nothing PKI-critical (Merkle works against the flat store). New `UnsupportedBackendError` type for callers that need to programmatically handle unavailable backends. +23 tests: 17 on the store primitive (upsert/remove/search/entries semantics) + 6 on persistence (roundtrip, extraMeta visibility, version-mismatch rejection, orphan compaction, clearPersisted, null-sidecar no-op).
- **PKI — `SemanticRetriever` migration to symbol index** (v0.62 step c.1, first of the v0.61 PKI deferrals). `SemanticRetriever.retrieve()` now prefers symbol-level hits from the [`SymbolEmbeddingIndex`](src/config/symbolEmbeddingIndex.ts) over the legacy file-level `rankFiles` path when PKI is wired + ready + has at least one symbol indexed. Symbol hits emit with a new `workspace-sym:${filePath}::${qualifiedName}` ID prefix so RRF fusion distinguishes them from legacy `workspace:${filePath}` hits; content renders the symbol body (line range extracted from `loadFileContent`) instead of the first 3000 chars of the whole file — a tighter evidence unit for RAG. File-level ranking stays as the fallback path, so pre-PKI behavior is preserved when PKI is disabled, still warming, or has 0 entries. Importantly, when the symbol index is populated but `search` returns no matches, the retriever returns `[]` (not a file-level fallback) — double-searching pollutes the fusion layer when symbol-level search legitimately had nothing to offer. New `WorkspaceIndex.setSymbolEmbeddings` + `getSymbolEmbeddings` hooks parallel the existing `setEmbeddingIndex` pair; extension.ts now wires the index into both the tool runtime (for `project_knowledge_search`) and the workspace index (for retrieval fusion). New `maxCharsPerSymbol` truncation knob (default 1500) on the retriever — smaller than `maxCharsPerFile` because symbol bodies are intrinsically more focused. +6 tests covering symbol-preferred path, file-level fallback when PKI not ready / empty, no-fallback-on-empty-search, symbol-body truncation, and unreadable-containing-file skip.
