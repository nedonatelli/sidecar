...
3. **Match the surrounding code** — comment density, naming, idiom. Comment only when the
   *why* is non-obvious; never narrate *what* the code does.
...
SideCar *is* an agent loop wired to fragile model runtimes, so "it compiles and unit tests
...
`jsonRepair` — to prove their tests *catch faults*, not just pass. It's on-demand (too slow
...
A model scoring 4/9 on evals is fine; a model *crashing the agent loop* is a release blocker.
...
- **Read trajectories, not pass/fail.** The failure *reason* and the event sequence (which
...
- **A bug found via eval is a real product bug.** Fix it in the product *and* add a fast unit
...
The eval harness (`tests/llm-eval/`) measures whether a model can *address the task*, played
