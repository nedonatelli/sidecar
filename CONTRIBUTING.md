...
| `ROADMAP.md`                   | Forward-looking only (July 2026 revector) — completed work lives in the CHANGELOG, never here. On release: **delete** the shipping version's row from the Planned table (its history is the CHANGELOG entry), move any deferred scope back to the Depth Backlog or Vision Shelf, delete the Depth Backlog items it closed, and record any new Prove-or-Prune Ledger verdicts (with the measured evidence — the Ledger is the one place a resolved item stays, as a verdict, not a history entry). |
