...
import { applyScopedAuthor } from './scopedAuthor.js';
 ...
 */
const scopedAuthorHook: PolicyHook = {
  name: 'scopedAuthor',
  async afterToolResults(state: LoopState, ctx: HookContext): Promise<HookResult> {
    if (!ctx.pendingToolUses || !ctx.toolResults) return { mutated: false };
    const mutated = await applyScopedAuthor(state, ctx.pendingToolUses, ctx.toolResults, ctx.client, ctx.callbacks);
    return { mutated };
  },
};
