...
export interface SideCarConfig {
  ...
  /** Notation-reconciliation note: appends a prompt section telling the model that the backtick `tool(arg=...)` snippets are shorthand for WHICH tool/fields — not the emission format — and that calls must go through the function-calling mechanism. Aimed at tools-native templates (qwen2.5-coder renders the catalog as a <tools> block demanding <tool_call> JSON) whose emission instruction our expression-style snippets may be teaching against — the format-confusion hypothesis behind qwen's code-as-text prose calls. Default OFF pending the qwen2.5-coder A/B. */
  nativeToolCallNoteEnabled: boolean;
...
function readConfig(): SideCarConfig {
  ...
  return {
    ...
    nativeToolCallNoteEnabled: cfg.get<boolean>('prompt.nativeToolCallNote', false),
