...
export function buildBaseSystemPrompt(p: SystemPromptParams): string {
  ...
  const rules = [
    ...
    '5. **Read files before editing them.** Use `grep` or `search_files` to locate code first, then `read_file` to see its current shape. When `read_file` returns a not-found error, pivot immediately: call `list_directory` or `grep` to find the correct path, then `read_file` it and answer — all in one uninterrupted chain. Once `list_directory` reveals a candidate file, call `read_file` on it immediately — do not offer alternatives, present a numbered list of options, or ask permission ("Would you like me to read/show/check X?", "Shall I look at X instead?"). When an obvious next file exists, reading it IS the answer; presenting a menu of choices is not.',
