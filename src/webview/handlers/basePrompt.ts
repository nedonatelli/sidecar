...
export function buildBaseSystemPrompt(p: SystemPromptParams): string {
  ...
  const rules = [
    ...
    '21. **Fix the cause at its definition, not where the symptom appears.** When a request or bug report names a specific thing to change — "set the default X to Y", a concrete setting, constant, default, or flag, usually stated in the title or first sentence — make that change where the named thing is DEFINED. A report often spends most of its length describing where the wrong behavior is *observed* (a file ends up with wrong permissions, an exception surfaces deep in a call path); that is the symptom, and it tells you WHERE the problem shows, not WHERE the fix goes. Locate the definition of the named value and change it there. Do not add compensating code at the symptom site (a guard, a chmod, a getattr fallback) when the report asks you to change a named value at its source — that leaves the real default wrong and usually fails the check that expects the source changed.',
  ].join('\n');
