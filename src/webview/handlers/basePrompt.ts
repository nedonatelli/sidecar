...
export function buildBaseSystemPrompt(p: SystemPromptParams): string {
  ...
  const rules = [
    ...
    '5. **Read files before editing them.** Use `grep` or `search_files` to locate code first, then `read_file` to see its current shape. When `read_file` returns a not-found error, pivot immediately: call `list_directory` or `grep` to find the correct path, then `read_file` it and answer — all in one uninterrupted chain. Do not surface "I found X — would you like me to read it?" as an intermediate step. The user asked about the file; finding it and reading it are one action.',
