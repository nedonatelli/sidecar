...
| --- | --------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| A1  | **Per-tool grammar from JSON schema** — generate GBNF (llama.cpp), use Ollama `format`, or XGrammar/Outlines via vLLM | Drive it off your existing tool registry; one grammar per tool, union grammar gated on selected tool name                                    | Schema validity rate → ~100% on local models                     | 🟡 — Ollama `format` schema field **plumbed but dead code**, zero callers (ollamaBackend.ts:565, client.ts:469); no GBNF/XGrammar   |
| A2  | **Constrain only at the action boundary**                                                                             | Free-form reasoning stays unconstrained; grammar engages only when emitting the tool call. Over-constraining reasoning lobotomizes the model | Reasoning quality unchanged vs unconstrained; calls always valid | ❌ — format only on `complete()`, never `streamChat()`                                                                              |
...
| A5  | **JSON-repair + reprompt fallback**                                                                                   | For any backend without logit access, repair malformed calls and reprompt with the _specific_ parse error before failing the turn            | Malformed-call failures that reach the user → ~0                 | ❌ — malformed calls silently dropped (textParsing.ts:74); reprompt fires only on _empty_ response (actionReprompt.ts) — extendable |
...
- **Grammar-constrained decoding (A1–A2, A4)** — still the standout lever, but **A1 is half-wired, not greenfield**: the Ollama schema-`format` path exists as dead code (zero callers). First move is to activate it at the tool-call boundary, not build it.
- **JSON-repair / parse-error reprompt (A5)** — confirmed absent; malformed calls are silently dropped. `actionReprompt.ts` already reprompts on empty responses — extend it to parse errors. Cheap, high-value.
